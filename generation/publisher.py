...
class NarrativePublisher:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        logger.info(f"Publisher initialized: evidence_days={self.evidence_days}, "
    ...
    def get_narrative_cluster_ids(self, conn, narrative_id: int, days_back: int) -> List[str]:
        ...
        try:
            ...
            cluster_ids = [row[0] for row in cursor.fetchall() if row[0] is not None and row[0] != '']
            cursor.close()
            logger.debug(f"Found {len(cluster_ids)} cluster_ids for narrative {narrative_id}")
            return cluster_ids
    ...
    def get_cluster_evidence(self, conn, cluster_ids: List[str], days_back: int) -> Dict:
        ...
        cursor.execute(query, (cluster_ids, days_back, self.min_articles, self.min_sources))
        clusters = cursor.fetchall()

        evidence = {
            'qualifying_clusters': len(clusters),
            'max_cluster_size': max([c['size'] for c in clusters]) if clusters else 0,
            'max_source_count': max([c['source_count'] for c in clusters]) if clusters else 0,
            'latest_cluster_date': clusters[0]['latest_cluster_date'] if clusters else None
        }
    ...
    def check_parent_evidence(self, conn, parent_id: int) -> Dict:
        ...
        parent_cluster_ids = self.get_narrative_cluster_ids(conn, parent_id, self.parent_days)
        
        if not parent_cluster_ids:
            return {'parent_qualifying_clusters': 0}

        # Check evidence on parent's clusters
        parent_evidence = self.get_cluster_evidence(conn, parent_cluster_ids, self.parent_days)
        
        return {'parent_qualifying_clusters': parent_evidence['qualifying_clusters']}

    def calculate_cluster_entropy(self, conn, cluster_ids: List[int], days_back: int) -> float:
    ...
    def check_evidence_gates(self, conn, candidate: Dict) -> Tuple[bool, Dict]:
        ...
        cluster_ids = self.get_narrative_cluster_ids(conn, narrative_id, self.evidence_days)
        ...
        parent_evidence_sufficient = parent_evidence.get('parent_qualifying_clusters', 0) >= 2
        ...
        gate_results = {
            ...
            'parent_qualifying_clusters': parent_evidence.get('parent_qualifying_clusters', 0),
            'entropy': entropy,
    ...
    def promote_to_published(self, conn, candidate: Dict, gate_results: Dict):
        ...
        try:
            ...
            conn.rollback()
            logger.error(f"Failed to publish narrative {candidate['narrative_id']}: {e}")
            raise
        finally:
            cursor.close()
    ...
    def run_publication_review(self) -> Dict[str, int]:
        ...
        with self.get_db_connection() as conn:
            ...
            for candidate in candidates:
                ...
                evidence_passed, evidence_results = self.check_evidence_gates(conn, candidate)
                ...
                if evidence_passed and content_passed and safety_passed:
                    ...
                    if not evidence_passed:
                        ...
                        logger.info(f"  {narrative_id}: Evidence gates failed "
    ...
    def print_summary(self, results: Dict[str, int]):
        ...
        active_candidates = results['candidates_reviewed'] - results['already_published']
...
def main():
    ...
    parser.add_argument(
        "--entropy-max", type=float, default=2.40,
    ...
    sys.exit(0 if results['published'] > 0 or results['candidates_reviewed'] == 0 else 1)
