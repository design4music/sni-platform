...
class NarrativeCardGenerator:
    ...
    def extract_cluster_evidence(self, activity_timeline) -> Dict:
        ...
        try:
            ...
            elif isinstance(timeline_data, dict) and 'cluster_evidence' in timeline_data:
    ...
    def generate_enhanced_title(self, narrative: Dict, evidence: Dict) -> str:
        ...
        if len(words) < 8:
            ...
            strategic_terms = ['strategic', 'military', 'diplomatic', 'economic', 'sanctions', 'negotiations']
            ...
            for excerpt in existing_excerpts[:5]:  # Limit to 5
                if isinstance(excerpt, dict):
                    enhanced_excerpts.append({
                        ...
                        'quote': excerpt.get('quote', excerpt.get('text', ''))[:200]  # Truncate long quotes
                    })
    ...
    def calculate_source_stats(self, narrative: Dict, evidence: Dict) -> Dict:
        ...
        return {
            ...
            'quality_score': min(1.0, evidence['total_articles'] / 10.0),  # Simple quality heuristic
    ...
    def update_narrative_progress(self, conn, narrative_id: int, updates: Dict) -> bool:
        ...
        try:
            ...
            gen1_entry = {
                ...
                'summary_sentences': len([s for s in updates['summary'].split('.') if s.strip()]),
    ...
    def process_narrative(self, conn, narrative: Dict) -> bool:
        ...
        try:
            ...
            summary_sentences = len([s for s in structured_summary.split('.') if s.strip()])
            
            if title_words < 8 or title_words > 14:
                logger.warning(f"Title word count ({title_words}) outside 8-14 range for narrative {narrative_id}")
            
            if summary_sentences < 2:
                logger.warning(f"Summary has only {summary_sentences} sentences for narrative {narrative_id}")
...
def main():
    ...
    parser.add_argument('--force', action='store_true',
                        help='Force regeneration of all cards')
