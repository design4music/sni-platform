...
class NarrativeEnricher:
    ...
    def __init__(self, batch_size: int = 20, force: bool = False):
        ...
        self.stance_indicators = {
            'pro': ['supports', 'endorses', 'welcomes', 'praises', 'approves', 'backs', 'champions'],
            'anti': ['opposes', 'condemns', 'rejects', 'criticizes', 'denounces', 'protests', 'disputes'],
            'neutral': ['reports', 'states', 'announces', 'confirms', 'indicates', 'reveals', 'shows']
        }
        
        self.actor_patterns = {
            'countries': ['United States', 'China', 'Russia', 'European Union', 'Germany', 'France', 'UK', 'Japan', 'India'],
            'organizations': ['NATO', 'UN', 'World Bank', 'IMF', 'WHO', 'WTO', 'OPEC'],
            'leaders': ['President', 'Prime Minister', 'Chancellor', 'Secretary', 'Minister', 'Director']
        }
        
        self.causal_indicators = [
            'because of', 'due to', 'as a result', 'leading to', 'causing', 'triggered by',
            'in response to', 'following', 'after', 'prompted by', 'resulting in'
        ]
    ...
    def analyze_alignment(self, narrative: Dict) -> Dict:
        ...
        for actor in self.actor_patterns['countries'] + self.actor_patterns['organizations']:
        ...
        for stance, indicators in self.stance_indicators.items():
            stance_scores[stance] = sum(1 for indicator in indicators if indicator in text)
    ...
    def extract_actor_origin(self, narrative: Dict) -> List[str]:
        ...
        leader_pattern = r'(President|Prime Minister|Chancellor|Secretary|Minister)\s+(\w+)'
    ...
    def analyze_frame_logic(self, narrative: Dict) -> Dict:
        ...
        if causal_matches:
            ...
            parts = re.split('|'.join(self.causal_indicators), longest_match, flags=re.IGNORECASE)
            if len(parts) >= 2:
    ...
    def extract_turning_points(self, narrative: Dict) -> List[Dict]:
        ...
        if activity_timeline:
            try:
                ...
                elif isinstance(activity_timeline, dict) and 'cluster_evidence' in activity_timeline:
                        ...
                        try:
                            if isinstance(timestamp, str):
                                ...
                                turning_points.append({
                                    ...
                                    'significance': min(1.0, entry.get('size', 1) / 10.0)
                                })
                        except ValueError:
                            continue
    ...
    def calculate_narrative_tension(self, narrative: Dict) -> float:
        ...
        # Tension indicators
        high_tension_words = ['crisis', 'conflict', 'war', 'sanctions', 'threatens', 'escalates', 'clash']
        medium_tension_words = ['tensions', 'disputes', 'concerns', 'challenges', 'pressure', 'confronts']
        tension_words = ['disagree', 'opposition', 'criticism', 'protests', 'resistance']
    ...
    def calculate_logical_strain(self, narrative: Dict) -> float:
        ...
        strain_indicators = ['however', 'but', 'contradicts', 'despite', 'although', 'while', 'unclear', 'disputed']
        coherence_indicators = ['therefore', 'thus', 'consequently', 'as a result', 'leading to', 'because']
    ...
    def update_narrative_enrichment(self, conn, narrative_id: int, enrichment: Dict) -> bool:
    ...
    def process_narrative(self, conn, narrative: Dict) -> bool:
        ...
        try:
            ...
            if alignment['indicators_found'] == 0:
                logger.warning(f"No stance indicators found for narrative {narrative_id}")
            ...
            if self.update_narrative_enrichment(conn, narrative_id, enrichment):
                ...
                logger.info(f"SUCCESS: Enriched narrative {narrative_id} (tension: {narrative_tension}, strain: {logical_strain})")
                return True
            else:
                self.error_count += 1
                return False
...
def main():
    ...
    parser.add_argument('--force', action='store_true',
                        help='Force regeneration of all enrichments')
