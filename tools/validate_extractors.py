...
class ExtractorValidator:
    ...
    async def validate_feed_profile(self, feed_info: Dict, days: int = 7, 
                                  sample_size: int = 5) -> Dict:
        """Validate a single feed's extraction profile"""
        feed_id = feed_info["id"]
        feed_url = feed_info["url"]
        profile = json.loads(feed_info["extraction_profile"]) if isinstance(feed_info["extraction_profile"], str) else feed_info["extraction_profile"]
        ...
        if len(recent_articles) < sample_size:
            logger.warning(f"Not enough recent articles for {feed_url}: {len(recent_articles)} < {sample_size}")
            return {
                "feed_id": feed_id,
                "feed_url": feed_url,
                "success": False,
        ...
        passes, validation_results = self.learner.validate_profile(profile, html_samples)
        
        # Calculate metrics
        successful_results = [r for r in validation_results["results"] if r.get("success", False)]
    ...
    def needs_relearning(self, validation_result: Dict) -> bool:
        ...
        if validation_result["success_rate"] < 0.7:
            logger.info(f"Profile needs relearning: success rate {validation_result['success_rate']:.1%} < 70%")
            return True
        ...
        length_drop = (baseline_length - current_length) / baseline_length if baseline_length > 0 else 0
        density_drop = (baseline_density - current_density) / baseline_density if baseline_density > 0 else 0
        
        if length_drop > 0.4:
            logger.info(f"Profile needs relearning: length dropped {length_drop:.1%} > 40%")
            return True
        
        if density_drop > 0.4:
            logger.info(f"Profile needs relearning: density dropped {density_drop:.1%} > 40%")
            return True
    ...
    async def auto_relearn_profile(self, feed_info: Dict) -> bool:
        ...
        try:
            if scope == "domain":
                ...
                success = await self.learner.learn_profile(domain=domain, sample_size=8, dry_run=False)
            else:
                success = await self.learner.learn_profile(feed_id=feed_id, sample_size=8, dry_run=False)
    ...
    async def validate_all_profiles(self, days: int = 7, sample_size: int = 5, 
                                  auto_relearn: bool = False) -> Dict:
        ...
        for feed_info in profiled_feeds:
            try:
                ...
                validation_result = await self.validate_feed_profile(feed_info, days, sample_size)
        ...
        summary = {
            ...
            "success_rate": successful_validations / total_feeds if total_feeds > 0 else 0,
            "relearned_count": relearned_count,
            "results": results
        }
        
        logger.info(f"Validation complete: {successful_validations}/{total_feeds} successful ({summary['success_rate']:.1%})")
        if auto_relearn:
...
async def main():
    parser = argparse.ArgumentParser(description="Validate extraction profiles and detect drift")
    parser.add_argument("--days", type=int, default=7, help="Fresh content window in days")
    parser.add_argument("--sample", type=int, default=5, help="Sample size for validation")
    parser.add_argument("--auto-relearn", action="store_true", 
    ...
    if args.feed_id:
        ...
        with get_db_session() as db:
            ...
            validation_result = await validator.validate_feed_profile(feed_dict, args.days, args.sample)
        ...
        summary = await validator.validate_all_profiles(args.days, args.sample, args.auto_relearn)
