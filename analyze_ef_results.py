...
def analyze_event_families():
    ...
    with get_db_session() as session:
        ...
        recent_efs = session.execute(text("""
        ...
        for ef in recent_efs:
            actors_str = ', '.join(ef.key_actors[:3]) + ('...' if len(ef.key_actors) > 3 else '')
        ...
        merged_efs = session.execute(text("""
        ...
        ef_key_analysis = session.execute(text("""
        ...
        efficiency = (stats.assigned_titles / stats.total_strategic_titles) * 100 if stats.total_strategic_titles > 0 else 0
        ...
        theater_analysis = session.execute(text("""
        ...
        for theater in theater_analysis:
            print(f"{theater.primary_theater}: {theater.ef_count} EFs, {theater.total_titles} titles")
