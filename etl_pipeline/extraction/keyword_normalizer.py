...
class KeywordNormalizer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        self.config_path = config_path or Path(__file__).parent.parent.parent / "data" / "keyword_synonyms.yml"
        ...
        self.stop_regex_patterns = [re.compile(pattern, re.IGNORECASE) 
                                   for pattern in self.config.get('stop_regex', [])]
        self.keeper_regex_patterns = [re.compile(pattern, re.IGNORECASE) 
...
if __name__ == "__main__":
    ...
    for text in test_cases:
        ...
        print(f"'{text}' -> '{norm}' -> '{canon}' (conf: {conf:.1f}) [cluster: {cluster}]")