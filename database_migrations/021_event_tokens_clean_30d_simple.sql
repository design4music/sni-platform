-- Create simplified event_tokens_clean_30d materialized view
...
-- Create the clean event tokens materialized view (simplified version)
...
shared_lib AS (
    ...
    SELECT tok FROM shared_keywords_lib_norm_30d
),
event_candidates AS (
    -- Start with base event tokens from auto-discovery
    SELECT 
        et.token,
        2 as country_cooccur,  -- Simplified for MVP
        7 as days_active       -- Simplified for MVP
...
filtered_events AS (
    ...
    WHERE 
        ...
        AND ec.token NOT IN ('business', 'company', 'companies', 'market', 'markets', 'economic', 'economy')  -- Generic business terms
        AND ec.token NOT IN ('time', 'year', 'years', 'month', 'months', 'week', 'weeks', 'day', 'days')  -- Time terms
)
SELECT 
    token,
    country_cooccur,
