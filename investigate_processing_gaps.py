...
def investigate_processing_gaps():
    ...
    with get_db_session() as session:
        ...
        unprocessed_sample = session.execute(text("""
            SELECT 
                id,
                title_display,
                publisher_name,
        ...
        batch_analysis = session.execute(text("""
            SELECT 
                processing_notes,
        ...
        lang_analysis = session.execute(text("""
            SELECT 
                detected_language,
        ...
        for lang in lang_analysis:
            print(f"  {lang.detected_language}: {lang.processed_count}/{lang.total_count} ({lang.processing_rate}%)")
        
        # Date distribution
        print("\n4. TEMPORAL DISTRIBUTION")
        print("-" * 50)
        
        date_analysis = session.execute(text("""
            SELECT 
        ...
        for date_row in date_analysis:
            rate = (date_row.processed_count / date_row.total_count * 100) if date_row.total_count > 0 else 0
            print(f"  {date_row.pub_date}: {date_row.processed_count}/{date_row.total_count} ({rate:.1f}%)")
        
        # Check for fragmentation in similar EFs
        print("\n5. EVENT FAMILY FRAGMENTATION ANALYSIS")
        print("-" * 50)
        
        fragmentation_check = session.execute(text("""
            SELECT 
                primary_theater,
                event_type,
                COUNT(*) as ef_count,
        ...
        trump_efs = session.execute(text("""
            SELECT 
                id,
                title,
                key_actors,
