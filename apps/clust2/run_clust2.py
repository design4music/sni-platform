...
def run_clust2(hours_back: int, dry_run: bool = False, summary: bool = False) -> dict:
    """
    ...
    print(f"Config: max_span={config.bucket_max_span_hours}h, min_size={config.bucket_min_size}, max_actors={config.bucket_max_actors}")
    ...
    recent_titles = bucket_manager.filter_recent_titles(titles)
    print(f"Filtered to {len(recent_titles)} recent titles")
    ...
    if summary and bucket_candidates:
        ...
        print(f"  Bucket sizes - min: {min(sizes)}, median: {sizes[len(sizes)//2] if sizes else 0}, p95: {sizes[int(0.95*len(sizes))] if sizes else 0}")
        print(f"  Time spans - min: {min(spans):.1f}h, median: {spans[len(spans)//2]:.1f}h, max: {max(spans):.1f}h")
        ...
        if len(recent_titles) > 0:
            ...
            rejected_pct = ((total_groups - accepted_groups) / total_groups * 100) if total_groups > 0 else 0
            print(f"  Rejection rate: {rejected_pct:.1f}% ({total_groups - accepted_groups}/{total_groups} groups)")
    else:
        for bucket in bucket_candidates:
            print(f"  {bucket.bucket_id}: {bucket.members_count} titles, {bucket.span_hours:.1f}h span, actors: {bucket.actor_set}")
...
def main():
    ...
    parser.add_argument('--hours', type=int, default=72,
                       help='Hours to look back for strategic titles (default: 72)')
    ...
    try:
        results = run_clust2(hours_back=args.hours, dry_run=args.dry_run, summary=args.summary)
