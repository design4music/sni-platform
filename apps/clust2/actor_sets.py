...
Uses shared taxonomy extractor with go/stop list awareness for bucket grouping
"""
...
class ActorSetBuilder:
    """Builds strategic actor sets for CLUST-2 bucket grouping with stop list filtering"""

    def __init__(self):
        self._extractor = create_multi_vocab_taxonomy_extractor()

    def extract_actor_set(self, title: Dict[str, Any]) -> List[str]:
        """
        Extract strategic actors only (blocked by stop lists) for bucket grouping.
        ...
        Logic:
        - If stop_culture.csv matches → return [] (no clustering)
        - Otherwise return all strategic actors from go lists
        """
        # Get text to analyze
        ...
        # Extract strategic actors with stop list blocking
        actor_codes = self._extractor.all_strategic_hits(title_text)

        # Optional: seed from gate for cheap wins (but still run all_strategic_hits to find others)
    ...
    def extract_and_build_key(
        self, title: Dict[str, Any], max_actors: int = 4
    ) -> tuple[List[str], str]:
        """
...
def extract_titles_actor_sets(
    titles: List[Dict[str, Any]], max_actors: int = 4
...
if __name__ == "__main__":
    ...
    try:
        ...
        actor_codes, bucket_key = builder.extract_and_build_key(
            test_title, max_actors=3
        )
