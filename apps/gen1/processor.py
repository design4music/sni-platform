...
class Gen1Processor:
    ...
    async def process_strategic_titles(
        ...
        Simplified: Process ALL unassigned strategic titles (event_family_id IS NULL)
        ...
        try:
            # Phase 1: Load ALL unassigned strategic titles (corpus-wide)
            ...
            for batch_idx, title_batch in enumerate(title_batches):
                logger.info(
                    f"Processing title batch {batch_idx + 1}/{len(title_batches)} ({len(title_batch)} titles)"
                )

                try:
                    # Create Event Families from title batch
                    ef_result = await self._process_title_batch_to_event_families(
                        title_batch
                    )
                    ...
                    if ef_result.event_families:
                        ...
                        for ef_data in ef_result.event_families:
                            event_family = await self._create_event_family_from_data(
                                ef_data
                            )
                            if event_family:
                                ...
                                fn_titles = [
                                    ...
                                    if str(t["id"])
                                    in ef_data.get("source_title_ids", [])
                                ]
                                fn_result = await self._process_event_family_to_framed_narratives(
                                    event_family, fn_titles
                                )

                                if fn_result.framed_narratives:
                                    for fn_data in fn_result.framed_narratives:
                                        framed_narrative = await self._create_framed_narrative_from_data(
                                            fn_data, event_family.id
                                        )
                                        if framed_narrative:
                                            all_framed_narratives.append(
                                                framed_narrative
                                            )
            ...
            success_rate = (
                len(all_event_families) / len(title_batches) if title_batches else 0.0
            )

            logger.info(
                "GEN-1 Direct Title Processing completed",
    ...
    async def _generate_framed_narratives(
        ...
        try:
            ...
            titles_context = await self._get_titles_context(
                event_family.source_title_ids
            )

            if not titles_context:
                logger.warning(
                    f"No title context found for Event Family {event_family.id}"
                )
                return []
    ...
    async def _process_title_batch_to_event_families(
        ...
        try:
            ...
            return await self.llm.assemble_event_families_from_titles(request)
    ...
    async def _process_event_family_to_framed_narratives(
        ...
        try:
            ...
            return await self.llm.generate_framed_narratives(request)
    ...
    async def _create_event_family_from_data(
        self, ef_data: Dict[str, Any]
    ) -> EventFamily:
        """
        ...
        try:
            framed_narrative = FramedNarrative(
                ...
                supporting_title_ids=[
                    str(tid) for tid in fn_data.get("supporting_title_ids", [])
                ],
            ...
            if await self.db.save_framed_narrative(framed_narrative):
                ...
                logger.error(
                    f"Failed to save Framed Narrative: {framed_narrative.frame_type}"
                )
                return None
    ...
    async def _get_titles_context(self, title_ids: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with get_db_session() as session:
                ...
                placeholders = ",".join(
                    [f":title_id_{i}" for i in range(len(title_ids))]
                )
                query = f"""
                SELECT 
                ...
                params = {
                    f"title_id_{i}": title_id for i, title_id in enumerate(title_ids)
                }
...
"""
    ...
    async def get_processing_summary(self, since_hours: int = 24) -> Dict[str, Any]:
        """
