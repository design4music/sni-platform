...
class Gen1Validator:
    ...
    def validate_event_family(
        ...
        if event_family.confidence_score < self.min_confidence_score:
            issues.append(
                f"Confidence score too low: {event_family.confidence_score:.2f}"
            )
        ...
        if len(set(event_family.source_title_ids)) != len(
            event_family.source_title_ids
        ):
        ...
        if not is_valid:
            logger.debug(
                f"Event Family validation failed: {event_family.id}", issues=issues
            )
    ...
    def validate_framed_narrative(
        ...
        # Required fields validation
        if (
            not framed_narrative.frame_type
            or len(framed_narrative.frame_type.strip()) == 0
        ):
            issues.append("Frame type is missing")

        if (
            not framed_narrative.frame_description
            or len(framed_narrative.frame_description.strip()) < 10
        ):
            issues.append("Frame description is too short or empty")

        if (
            not framed_narrative.stance_summary
            or len(framed_narrative.stance_summary.strip()) < 10
        ):
            issues.append("Stance summary is too short or empty")

        # Evidence validation
        if (
            not framed_narrative.supporting_headlines
            and not framed_narrative.supporting_title_ids
        ):
        ...
        if framed_narrative.prevalence_score < self.min_prevalence_score:
            issues.append(
                f"Prevalence score too low: {framed_narrative.prevalence_score:.2f}"
            )

        if framed_narrative.evidence_quality < 0.2:
            issues.append(
                f"Evidence quality too low: {framed_narrative.evidence_quality:.2f}"
            )
        ...
        if (
            ...
            and len(framed_narrative.supporting_title_ids)
            != len(framed_narrative.supporting_headlines)
        ):
        ...
        if not is_valid:
            logger.debug(
                f"Framed Narrative validation failed: {framed_narrative.id}",
                issues=issues,
            )
    ...
    def validate_processing_result(
        ...
        valid_items = (
            result["event_families"]["valid"] + result["framed_narratives"]["valid"]
        )
    ...
    def _generate_recommendations(self, validation_result: Dict[str, Any]) -> List[str]:
        ...
        if ef_result["invalid"] > ef_result["valid"]:
            recommendations.append(
                "Consider raising confidence thresholds for Event Family assembly"
            )

        if "Coherence reason is too brief" in ef_result["issues"]:
            recommendations.append(
                "Improve LLM prompting for more detailed coherence explanations"
            )

        # Framed Narrative recommendations
        if fn_result["invalid"] > fn_result["valid"]:
            recommendations.append(
                "Consider improving evidence collection for Framed Narratives"
            )
        ...
        if quality_score < 0.5:
            recommendations.append(
                "Overall quality is low - review LLM prompts and validation thresholds"
            )
        elif quality_score < 0.8:
            recommendations.append(
                "Quality is moderate - consider fine-tuning processing parameters"
            )
    ...
    def get_quality_metrics(
        ...
        if event_families:
            ...
            metrics["event_families"] = {
                ...
                "avg_actors": sum(len(ef.key_actors) for ef in event_families)
                / len(event_families),
                "avg_source_titles": sum(
                    len(ef.source_title_ids) for ef in event_families
                )
                / len(event_families),
            }
        ...
        if framed_narratives:
            ...
            metrics["framed_narratives"] = {
                ...
                "avg_supporting_headlines": sum(
                    len(fn.supporting_headlines) for fn in framed_narratives
                )
                / len(framed_narratives),
                "unique_frame_types": len(
                    set(fn.frame_type for fn in framed_narratives)
                ),
            }
