...
class EventFamily(BaseModel):
    ...
    event_type: str = Field(description="Type of event (e.g., 'diplomatic meeting', 'economic policy')")
    geography: Optional[str] = Field(default=None, description="Geographic location if relevant")
    
    # Time boundaries
    event_start: datetime = Field(description="When the event began")
    event_end: Optional[datetime] = Field(default=None, description="When the event concluded (if applicable)")
    
    # Source metadata
    source_bucket_ids: List[str] = Field(description="CLUST-2 bucket IDs that contributed to this EF")
    source_title_ids: List[str] = Field(description="Title IDs that are part of this event family")
    
    # Quality indicators
    confidence_score: float = Field(ge=0.0, le=1.0, description="LLM confidence in event coherence")
...
class FramedNarrative(BaseModel):
    ...
    frame_type: str = Field(description="Type of framing (e.g., 'supportive', 'critical', 'neutral')")
    frame_description: str = Field(description="How this narrative frames the event")
    stance_summary: str = Field(description="Clear statement of the evaluative/causal framing")
    
    # Evidence and support
    supporting_headlines: List[str] = Field(description="Headlines that exemplify this framing")
    supporting_title_ids: List[str] = Field(description="Title IDs that support this narrative")
    key_language: List[str] = Field(description="Key words/phrases that signal this framing")
    
    # Narrative strength
    prevalence_score: float = Field(ge=0.0, le=1.0, description="How dominant this framing is")
