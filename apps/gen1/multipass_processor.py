...
class MultiPassProcessor:
    ...
    async def run_pass1_entity_assembly(
        ...
        try:
            ...
            # Use batch_size=1000 to respect LLM context limits (~131k tokens)
            sequential_batches = group_titles_sequentially(titles, batch_size=1000)
                ...
                try:
                    ...
                    for ef_data in ef_results.get("event_families", []):
                        ...
                        event_family = EventFamily(
                            ...
                            event_end=datetime.fromisoformat(ef_data["event_end"]) if ef_data.get("event_end") else None,
                        ...
                        if not dry_run:
                            ...
                            if await self.db.save_event_family(event_family):
                                logger.debug(f"Pass 1: Saved EF: {event_family.title}")
                                ...
                                processing_errors.append(f"Failed to save Event Family: {event_family.title}")

                        all_event_families.append(event_family)

                except Exception as e:
                    logger.error(f"Pass 1: Error processing sequential batch {batch_key}: {e}")
                    processing_errors.append(f"Sequential batch {batch_key}: {e}")
                    continue

            # Calculate final stats
            processing_time = (datetime.now() - start_time).total_seconds()
            total_processed = sum(batch_info['size'] for batch_info in sequential_batches)
            success_rate = len(all_event_families) / max(total_processed, 1) if all_event_families else 0.0
    ...
    async def _generate_pass1_event_families(self, titles: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for i, title in enumerate(titles):
            titles_context.append({
                "headline_id": title["id"],  # Use actual title ID instead of fake headline{i}
                "headline": title["text"],
                "source": title.get("source", "Unknown"),
                "pubdate": title.get("pubdate_utc", "").strftime("%Y-%m-%d %H:%M") if title.get("pubdate_utc") else "Unknown",
                "actors": title.get("actors", {}).get("actors", []) if isinstance(title.get("actors"), dict) else [],
            })

        prompt = f"""You are assembling Event Families (EFs) from a diverse batch of strategic headlines.

PASS 1 OBJECTIVE: Create basic Event Families with essential metadata only. 
...
INSTRUCTIONS:
1. Group headlines that describe the same real-world event/happening across ANY topic or geography
2. Focus on concrete events with clear actors, timing, and coherent narrative
3. Create 5-15 Event Families maximum (process this large diverse batch efficiently)
4. Provide BASIC metadata only:
   - Clear, descriptive title
   - Factual summary (2-3 sentences)
   - Key actors involved
   - Event type classification
   - Geographic location if relevant
   - Time boundaries (start/end dates from headlines)
   - Which headlines belong to this EF
   - Confidence score (0.0-1.0)
   - Brief coherence reasoning
...
{{
    "event_families": [
        {{
            ...
            "source_title_ids": ["headline0", "headline1"],
            "confidence_score": 0.85,
            "coherence_reason": "Brief explanation of why these headlines form a coherent event"
        }}
    ]
}}

Remember: This is Pass 1 - keep it simple and focused on basic EF assembly."""
        ...
        try:
            ...
            response = await self.llm_client.assemble_event_families_from_titles(request)
    ...
    def _format_titles_for_llm(self, titles_context: List[Dict[str, Any]]) -> str:
        ...
        for title in titles_context:
            actors_str = ", ".join(title.get("actors", [])) if title.get("actors") else "None"
            formatted.append(
    ...
    async def run_pass2_cross_merging(
        ...
        try:
            ...
            event_families = await self.db.get_event_families(
                since_hours=48,  # Look at last 2 days
                limit=max_event_families
            )
            ...
            logger.info(f"Found {len(event_families)} Event Families for Pass 2 analysis")

            # Analyze for merge opportunities
            merged_families = await self._analyze_merge_opportunities(event_families, dry_run)
            ...
            if generate_narratives:
                ...
                families_to_process = merged_families if merged_families else event_families[:10]  # Limit for testing
                
                for i, event_family in enumerate(families_to_process, 1):
                    logger.info(f"Pass 2: Generating narratives for EF {i}/{len(families_to_process)}: {event_family.title}")
                    
                    try:
                        # Get associated titles for this Event Family
                        titles_context = await self._get_titles_for_event_family(event_family)
                        
                        if len(titles_context) < 2:
                            processing_warnings.append(f"EF '{event_family.title}' has insufficient titles for narrative analysis")
                            continue
                        
                        # Generate Framed Narratives
                        narratives = await self._generate_framed_narratives(event_family, titles_context)
                        
                        for narrative in narratives:
                            if not dry_run:
                                if await self.db.save_framed_narrative(narrative):
                                    logger.debug(f"Saved Framed Narrative: {narrative.frame_type}")
                                else:
                                    processing_errors.append(f"Failed to save narrative for EF: {event_family.title}")
                            
                            all_narratives.append(narrative)
                            
                    except Exception as e:
                        logger.error(f"Pass 2: Error generating narratives for EF {event_family.title}: {e}")
                        processing_errors.append(f"EF {event_family.title}: {e}")
                        continue

            # Calculate final stats
            processing_time = (datetime.now() - start_time).total_seconds()
            success_rate = len(all_narratives) / max(len(event_families), 1) if all_narratives else 0.0
    ...
    async def _analyze_merge_opportunities(self, event_families: List[EventFamily], dry_run: bool) -> List[EventFamily]:
        """
        ...
        """
        logger.info(f"Analyzing {len(event_families)} Event Families for merge opportunities")
        ...
        try:
            ...
            for i, ef in enumerate(event_families):
                ef_context.append({
                    ...
                    "event_start": ef.event_start.strftime("%Y-%m-%d %H:%M") if ef.event_start else "Unknown",
                    "event_end": ef.event_end.strftime("%Y-%m-%d %H:%M") if ef.event_end else "Unknown",
            ...
            if merge_analysis.get("merge_groups"):
                ...
                for group_info in merge_analysis["merge_groups"]:
                    ...
                    families_to_merge = [event_families[i] for i in ef_indices if i < len(event_families)]
                    
                    if len(families_to_merge) < 2:
                        continue
                    
                    # Create merged Event Family
                    merged_ef = await self._merge_event_families(families_to_merge, group_info, dry_run)
                    if merged_ef:
                        merged_families.append(merged_ef)
                        logger.info(f"Merged {len(families_to_merge)} EFs into: {merged_ef.title}")
            ...
            logger.info(f"Pass 2 merging: {len(event_families)} → {len(merged_families)} Event Families")
    ...
    async def _get_titles_for_event_family(self, event_family: EventFamily) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            with get_db_session() as session:
                ...
                uuid_list = "ARRAY[" + ",".join([f"'{title_id}'::uuid" for title_id in event_family.source_title_ids]) + "]"
                ...
                for row in results:
                    titles_context.append({
                        ...
                        "text": row.text,
                        "source": row.source,
                        "pubdate": row.pubdate_utc.strftime("%Y-%m-%d %H:%M") if row.pubdate_utc else "Unknown",
                        "language": row.language,
                    })
                
                logger.debug(f"Retrieved {len(titles_context)} titles for EF: {event_family.title}")
                return titles_context
                
        except Exception as e:
            logger.error(f"Failed to get titles for Event Family {event_family.id}: {e}")
            return []

    async def _generate_framed_narratives(self, event_family: EventFamily, titles_context: List[Dict[str, Any]]) -> List[FramedNarrative]:
        """
        ...
        try:
            ...
            logger.debug(f"Generated {len(narratives)} Framed Narratives for EF: {event_family.title}")
            return narratives
            
        except Exception as e:
            logger.error(f"Failed to generate Framed Narratives for EF {event_family.id}: {e}")
            return []

    async def _generate_merge_analysis(self, ef_context: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
...
"""
        ...
        try:
            ...
            logger.info(f"Merge analysis completed: {len(merge_analysis.get('merge_groups', []))} potential merges identified")
            
            return merge_analysis

        except Exception as e:
            logger.error(f"Merge analysis LLM call failed: {e}")
            return {"merge_groups": [], "standalone_event_families": list(range(len(ef_context)))}

    async def _merge_event_families(self, families_to_merge: List[EventFamily], group_info: Dict[str, Any], dry_run: bool) -> Optional[EventFamily]:
        """
        ...
        try:
            ...
            for ef in families_to_merge:
                ...
                if earliest_start is None or (ef.event_start and ef.event_start < earliest_start):
            ...
            merged_ef = EventFamily(
                title=group_info.get("merged_title", f"Merged: {families_to_merge[0].title}"),
                summary=group_info.get("merged_summary", "Consolidated Event Family from multiple sources"),
                key_actors=list(group_info.get("unified_actors", all_actors))[:10],  # Limit actors
                event_type=f"Consolidated: {families_to_merge[0].event_type}",
                geography=group_info.get("unified_geography", families_to_merge[0].geography),
                event_start=earliest_start,
                event_end=latest_end,
                source_title_ids=list(set(all_title_ids)),  # Deduplicate title IDs
                confidence_score=min(total_confidence / len(families_to_merge), group_info.get("confidence", 0.8)),
            ...
            if not dry_run:
                ...
                if await self.db.save_event_family(merged_ef):
                    ...
                    logger.info(f"Saved merged EF: {merged_ef.id} with {len(merged_ef.source_title_ids)} titles")
                else:
                    logger.error(f"Failed to save merged Event Family: {merged_ef.title}")
                    return None
    ...
    async def main():
        ...
        if len(sys.argv) > 1 and sys.argv[1] == "pass1":
            ...
            result = await processor.run_pass1_entity_assembly(
                ...
                batch_size=max_titles or 100,  # Use max_titles as batch_size for testing
            ...
            print("Usage: python -m apps.gen1.multipass_processor [pass1|pass2] [max_items] [--dry-run]")
            print("  pass1: Run Pass 1 (EF assembly from titles)")
            print("  pass2: Run Pass 2 (EF cross-merging and narrative generation)")
            print("  max_items: Maximum titles (pass1) or Event Families (pass2) to process")
