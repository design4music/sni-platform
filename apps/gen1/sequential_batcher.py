...
def group_titles_sequentially(titles: List[Dict[str, Any]], batch_size: int = 100) -> List[Dict[str, Any]]:
    """
...
def extract_batch_context(batch_titles: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Date range
    dates = [title.get('pubdate_utc') for title in batch_titles if title.get('pubdate_utc')]
    ...
    return {
        ...
        'date_span_hours': (latest_date - earliest_date).total_seconds() / 3600 if earliest_date and latest_date else 0,
    }
