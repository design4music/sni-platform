...
class RSSFetcher:
    ...
    def extract_real_publisher(self, entry, feed) -> Tuple[Optional[str], Optional[str]]:
        """
    ...
    def fetch_with_retries(self, feed_url: str, headers: Dict) -> Optional[requests.Response]:
        """
        ...
        for attempt in range(self.config.http_retries):
            try:
                ...
                if attempt == self.config.http_retries - 1:
                    logger.error(f"HTTP error after {self.config.http_retries} attempts for {feed_url}: {e}")
                    raise
                
                # Exponential backoff with jitter
                delay = (2 ** attempt) + random.uniform(0, 1)
                logger.warning(f"HTTP error attempt {attempt + 1}/{self.config.http_retries} for {feed_url}: {e}. Retrying in {delay:.1f}s")
                time.sleep(delay)
    ...
    def fetch_feed(self, feed_id: str, feed_url: str) -> Tuple[List[Dict], Dict]:
        ...
        stats = {'fetched': 0, 'inserted': 0, 'skipped': 0, 'errors': 0, 'duration_sec': 0}
        ...
        try:
            ...
            if feed_meta.get('last_pubdate_utc'):
                watermark_date = feed_meta['last_pubdate_utc'] - timedelta(days=self.config.lookback_days)
            ...
            for entry in feed.entries:
                try:
                    ...
                    if pubdate_utc and (not latest_pubdate or pubdate_utc > latest_pubdate):
                    ...
                    publisher_name, publisher_domain = self.extract_real_publisher(entry, feed)
                    ...
                    if self.config.max_items_per_feed and len(articles) >= self.config.max_items_per_feed:
                        break
            ...
            logger.info(f"Feed processing complete: {feed_url} - fetched: {stats['fetched']}, inserted: {stats['inserted']}, skipped: {stats['skipped']}, errors: {stats['errors']}, duration: {stats['duration_sec']:.2f}s")
