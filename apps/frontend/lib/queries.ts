...
// retired. Replaced by the per-outlet/per-entity/per-month stance matrix
// (table outlet_entity_stance, schema pending).
...
// D-071: getStanceMatrix + AlignmentRow retired (/sources/alignment page
...
export async function getCalendarMonthView(
  ...
  const monthStart = month.length === 7 ? `${month}-01` : month;
  const loc = locale === 'de' ? 'de' : 'en';
  return cached(`calendar:${centroidId}:${track}:${monthStart}:${loc}`, 12 * 3600, async () => {
    // Read pre-computed view from mv_calendar_month_view. Materializer
    // (pipeline/phase_4/materialize_calendar_month_view.py) runs every
    // 12h on the daemon worker; frontend just does a PK lookup.
    // Top-20-clusters-per-day cap is enforced inside the materializer.
    const rows = await query<{ view: CalendarMonthView }>(
      `SELECT view
         FROM mv_calendar_month_view
        WHERE centroid_id = $1 AND track = $2 AND month = $3::date AND locale = $4`,
      [centroidId, track, monthStart, loc]
    );
...
export async function getCentroidMonthView(
  ...
  const monthStart = month.length === 7 ? `${month}-01` : month;
  const loc = locale === 'de' ? 'de' : 'en';
  return cached(`centroid_cal:${centroidId}:${monthStart}:${loc}`, 12 * 3600, async () => {
    // Read pre-computed view from mv_centroid_month_view. Materializer
    // (pipeline/phase_4/materialize_centroid_month_view.py) runs every
    // 12h on the daemon worker; frontend just does a PK lookup.
    // The blob shape mirrors the CentroidMonthView interface exactly.
    const rows = await query<{ view: CentroidMonthView }>(
      `SELECT view
         FROM mv_centroid_month_view
        WHERE centroid_id = $1 AND month = $2::date AND locale = $3`,
      [centroidId, monthStart, loc]
    );
    return rows[0]?.view || null;
  });
}
...
export async function getGlobalMonthView(
  ...
  const monthStart = month.length === 7 ? `${month}-01` : month;
  const loc = locale === 'de' ? 'de' : 'en';
  return cached(`global_view:${monthStart}:${loc}`, 12 * 3600, async () => {
...
// scope.
// Reads from the same mv_global_month_view row as getGlobalMonthView —
// active_narratives is folded into that blob to avoid a second query.
// Materializer NARRATIVES_LIMIT (currently 10) caps the stored list.
export async function getActiveNarrativesGlobal(
  ...
  const monthStart = month.length === 7 ? `${month}-01` : month;
  const loc = locale === 'de' ? 'de' : 'en';
  return cached(`global_narratives:${monthStart}:${limit}:${loc}`, 12 * 3600, async () => {
    const rows = await query<{
      narratives: Array<{
        id: string;
        name: string;
      ...
      `SELECT view->'active_narratives' AS narratives
         FROM mv_global_month_view
        WHERE month = $1::date AND locale = $2`,
      [monthStart, loc]
    );
    return (rows[0]?.narratives || []).slice(0, limit);
  });
}
