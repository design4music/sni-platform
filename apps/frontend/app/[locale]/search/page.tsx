...
function TypeBadge({ type, labels }: { type: SearchResult['type']; labels: Record<string, string> }) {
  const colors = {
...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  const typeLabels = { event: t('topic'), centroid: t('country'), epic: t('epic') };
        ...
        {query && !error && (
          <div>
            <p className="text-sm text-dashboard-text-muted mb-4">
              {results.length === 0 ? t('noResults') : t('results', { count: results.length })} for &ldquo;{query}&rdquo;
            </p>
              ...
              {results.map(r => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <TypeBadge type={r.type} labels={typeLabels} />
                    {r.centroid_label && (
                    ...
                    {r.sources && (
                      <span className="text-xs text-dashboard-text-muted">{tCommon('sourcesCount', { count: r.sources })}</span>
                    )}
                  </div>
