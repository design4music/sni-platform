...
export default async function AnalysisPage({ params }: Props) {
  ...
  const trackLabel = n.track ? getTrackLabel(n.track, tTracks) : null;
      ...
      {n.centroid_id && n.centroid_name && (
        <>
          <Link href={`/c/${n.centroid_id}`} className="text-blue-400 hover:text-blue-300">
            {getCentroidLabel(n.centroid_id, n.centroid_name, tCentroids)}
          </Link>
    ...
    <div className="lg:sticky lg:top-24 space-y-4">
      ...
      {stats && stats.title_count > 0 && (
        <div className="bg-dashboard-border/30 rounded-lg p-4">
          <h3 className="text-sm font-semibold text-dashboard-text mb-3">{tAnalysis('coverageStats')}</h3>
          <div className="grid grid-cols-2 gap-3">
            <div className="text-center">
              <div className="text-xl font-bold text-dashboard-text">{stats.publisher_count}</div>
              <div className="text-xs text-dashboard-text-muted">{tAnalysis('publishers')}</div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-dashboard-text">{stats.language_count}</div>
              <div className="text-xs text-dashboard-text-muted">{tAnalysis('languages')}</div>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-dashboard-text-muted">{tAnalysis('dateSpan')}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {topPublishers.length > 0 && (
        <div className="bg-dashboard-border/30 rounded-lg p-4">
          <h3 className="text-sm font-semibold text-dashboard-text mb-3">{tAnalysis('topPublishers')}</h3>
      ...
      {topPersons.length > 0 && (
        <div className="bg-dashboard-border/30 rounded-lg p-4">
          <h3 className="text-sm font-semibold text-dashboard-text mb-3">{tAnalysis('topPersons')}</h3>
          <div className="space-y-1.5">
      ...
      {langVisible.length > 0 && (
        <div className="bg-dashboard-border/30 rounded-lg p-4">
          <h3 className="text-sm font-semibold text-dashboard-text mb-2">{tAnalysis('languages')}</h3>
          <div className="flex flex-wrap gap-1">
            ...
            {langHidden > 0 && (
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-dashboard-border text-dashboard-text-muted">
                {tAnalysis('moreCount', { count: langHidden })}
              </span>
            )}
          </div>
        </div>
      ...
      {geoSorted.length > 0 && (
        <div className="bg-dashboard-border/30 rounded-lg p-4">
          <h3 className="text-sm font-semibold text-dashboard-text mb-2">{tAnalysis('geographicFocus')}</h3>
          <div className="flex flex-wrap gap-1">
            {geoSorted.map(([code, count]) => (
              <span
                key={code}
                className="text-[10px] px-1.5 py-0.5 rounded bg-dashboard-border text-dashboard-text-muted"
                title={tAnalysis('mentions', { count })}
              >
                {getCountryName(code)}
              </span>
            ))}
