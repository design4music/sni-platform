...
from apps.clust1.strategic_gate import GateResult, filter_titles_batch
...
class GateProcessor:
    ...
    def update_gate_results(
        self, results: List[Tuple[Dict[str, Any], GateResult]]
    ) -> int:
        """
        ...
        for title, gate_result in results:
            ...
            updates.append({"title_id": title["id"], **update_data})
    ...
    def process_batch(self, titles: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            logger.info(
                f"Processing batch of {len(titles)} titles through Strategic Gate"
            )
    ...
    def run(self, max_batches: int = None) -> Dict[str, int]:
        ...
        while max_batches is None or batch_num < max_batches:
            ...
            logger.info(
                f"Processing batch {batch_num + 1}: {len(titles)} titles (offset={offset})"
            )
    ...
    def _log_summary(self):
        ...
        logger.info(
            f"Kept: {stats['kept']} ({stats['kept']/max(stats['total_processed'], 1)*100:.1f}%)"
        )
    ...
    def get_gate_summary(self) -> Dict[str, Any]:
        ...
        with self.Session() as session:
            ...
            reason_breakdown = {
                row.gate_reason: row.count for row in reason_result.fetchall()
            }
...
def main():
    ...
    parser.add_argument(
        "--summary", action="store_true", help="Show processing summary and exit"
    )
    parser.add_argument(
        "--pending", action="store_true", help="Show pending count and exit"
    )
    ...
    if args.summary:
        ...
        print(
            f"Gate Summary: {summary['kept_titles']}/{summary['processed_titles']} kept "
    ...
    try:
        ...
        print(
            f"GATE_RESULT: {stats['kept']}/{stats['total_processed']} kept, "
