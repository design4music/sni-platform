...
from apps.generate.multipass_processor import get_multipass_processor
...
class DailyContinuousMerging:
    ...
    async def run_daily_workflow(
        ...
        try:
            ...
            if unassigned_count == 0:
                ...
                logger.info(
                    f"Step 1: Running Pass 1 EF Assembly (max {max_titles_pass1} titles)"
                )
                ...
                if pass1_result.errors:
                    workflow_results["errors"].extend(
                        [f"Pass 1: {error}" for error in pass1_result.errors]
                    )
            ...
            logger.info(
                f"Found {active_ef_count} active Event Families for Pass 2A analysis"
            )
            ...
            if active_ef_count < 2:
                ...
                logger.info(
                    f"Step 2: Running Pass 2A EF Merging (max {max_efs_pass2a} EFs)"
                )
                pass2a_result = await self.processor.run_pass2a_ef_merging(
                    max_event_families=max_efs_pass2a, dry_run=dry_run
                )
                workflow_results["pass2a_results"] = pass2a_result

                if pass2a_result.errors:
                    workflow_results["errors"].extend(
                        [f"Pass 2A: {error}" for error in pass2a_result.errors]
                    )
            ...
            workflow_results["total_processing_time"] = (
            ...
            logger.info(
                f"Daily workflow completed in {workflow_results['total_processing_time']:.1f}s"
            )
    ...
    def _generate_workflow_summary(self, results: dict) -> str:
        ...
        if results["pass2a_results"]:
            ...
            summary_lines.append(
                f"Pass 2A: {len(p2a.event_families)} EFs after merging"
            )
        else:
    ...
    async def get_system_status(self) -> dict:
        ...
        try:
            with get_db_session() as session:
                ...
                metrics["unassigned_strategic_titles"] = (
                    session.execute(
                        text(
                            """
                ...
                metrics["theater_distribution"] = {
                    row.primary_theater: row.count for row in theater_dist
                }
...
async def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "status":
        ...
        if "metrics" in status:
            ...
            print(
                f"Unassigned Strategic Titles: {metrics['unassigned_strategic_titles']}"
            )
        ...
        results = await workflow.run_daily_workflow(
            max_titles_pass1=max_titles, max_efs_pass2a=max_efs, dry_run=dry_run
        )
        ...
        print(
            "Usage: python daily_continuous_merging.py [status|run] [max_titles] [max_efs] [--dry-run]"
        )
