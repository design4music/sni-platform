...
def generate_ef_key(actors: List[str], primary_theater: str, event_type: str) -> str:
    ...
    key_parts = [primary_theater.strip(), event_type.strip()]
    ...
    logger.debug(
        f"Generated ef_key: {ef_key} from key_string: {key_string} (actors ignored)"
    )
    return ef_key
...
def validate_ef_key_components(
    actors: List[str], primary_theater: str, event_type: str
) -> bool:
    """
...
def should_consider_for_merge(
    ef1_data: Dict[str, Any], ef2_data: Dict[str, Any]
) -> bool:
    """
    ...
    if generate_ef_key(actors1, theater1, type1) == generate_ef_key(
        actors2, theater2, type2
    ):
        return False
...
def test_ef_key_generation():
    ...
    logger.info(
        f"Test 5: Actor similarity {actors} vs {actors_different} = {similarity:.2f}"
    )
