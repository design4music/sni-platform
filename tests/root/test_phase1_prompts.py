...
async def test_phase1_fragmentation_reduction():
    ...
    logger.info(
        "Expected Reduction: Should be 2-3 broad EFs instead of 7+ fragmented ones"
    )
    ...
    single_title_efs = len(
        [ef for ef in result.event_families if len(ef.source_title_ids) == 1]
    )
    multi_title_efs = len(
        [ef for ef in result.event_families if len(ef.source_title_ids) > 1]
    )
    avg_titles_per_ef = (
        sum(len(ef.source_title_ids) for ef in result.event_families)
        / len(result.event_families)
