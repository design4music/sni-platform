...
async def test_phase2_bucketless():
    ...
    if not titles:
        logger.warning(
            "No unassigned strategic titles found - Phase 2 test cannot proceed"
        )
    ...
    result = await processor.process_strategic_titles(
        since_hours=72, max_titles=10, batch_size=10, dry_run=False
    )
    ...
    # Validate success
    success = (
        result.event_families_created > 0
        and result.framed_narratives_created > 0
    ...
    if success:
        ...
        logger.info(
            f"Remaining unassigned titles: {len(updated_titles)} (reduced by {len(titles) - len(updated_titles)})"
        )
...
if __name__ == "__main__":
    try:
        ...
        if success:
            ...
            logger.warning(
                "Do not proceed with bucket table cleanup until issues resolved"
            )
