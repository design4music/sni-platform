...
Generate parent and child narrative candidates following the specified format."""
    ...
    async def _store_narrative(
        ...
        """Store narrative in database with cluster evidence linking"""

        narrative_id = str(uuid.uuid4())

        with get_db_session() as session:
            # Get cluster metadata for evidence linking
            cluster_query = text("""
                SELECT 
                    ac.cluster_id,
                    ac.size,
                    COUNT(DISTINCT a.source_name) as source_count,
                    0 as has_event_signal  -- Simplified: skip event signal check for now
            ...
            cluster_result = session.execute(cluster_query, {"cluster_id": cluster_id}).fetchone()
            
            if cluster_result:
                # Create activity_timeline entry with cluster evidence
                activity_timeline = [{
                    "ts": datetime.now().isoformat(),
                    "stage": "clust1",
                    "cluster_id": str(cluster_id),
                    "size": int(cluster_result.size),
                    "sources": int(cluster_result.source_count),
                    "event_signal": bool(cluster_result.has_event_signal),
                    "action": f"narrative_created_{narrative_type}"
                }]
            else:
                # Fallback if cluster data not found
                activity_timeline = [{
                    "ts": datetime.now().isoformat(),
                    "stage": "clust1", 
                    "cluster_id": str(cluster_id),
                    "action": f"narrative_created_{narrative_type}_no_metadata"
                }]

            # Insert narrative with cluster evidence and initial publication status
            session.execute(
                text(
                    """
                INSERT INTO narratives (
                    id, narrative_id, title, summary, parent_id, consolidation_stage,
                    publication_status, activity_timeline, created_at, updated_at, 
                    origin_language, dominant_source_languages, alignment, actor_origin, 
                    conflict_alignment, frame_logic
                ) VALUES (
                    :id, :narrative_id, :title, :summary, :parent_id, 'raw',
                    'draft', :activity_timeline, :created_at, :updated_at, 'en', '{}', 
                    '{}', '{}', '{}', '{}'
                )
            """
                ),
                {
                    ...
                    "summary": summary,
                    "parent_id": parent_id,
                    "activity_timeline": json.dumps(activity_timeline),
        ...
        print(f"Stored {narrative_type} narrative {narrative_id} with cluster evidence for cluster {cluster_id}")
        return narrative_id
    ...
    async def process_clusters(
        ...
        # Module 1: Conservative Strategic Pre-Filtering  
        # Only mark clusters that pass conservative thresholds as strategic
        print("=== Module 1: Conservative Strategic Pre-Filtering ===")
        strategic_clusters = []
        ...
        for cluster in clusters:
            ...
            has_event_signal = cluster.get("has_event_signal", 0)
            
            # Apply conservative filters
            if cluster_size >= 4 and source_count >= 3:
                strategic_clusters.append(cluster)
                print(f"STRATEGIC: Cluster {cluster_id} - {cluster_size} articles, {source_count} sources, event: {bool(has_event_signal)}")
                
                # Mark as strategic in database
                with get_db_session() as session:
                    session.execute(
                        text(
                            """
                ...
                print(f"FILTERED OUT: Cluster {cluster_id} - {cluster_size} articles, {source_count} sources (below thresholds)")
                self.stats["non_strategic_count"] += 1
        
        self.stats["strategic_count"] = len(strategic_clusters)
        print(f"Conservative filtering: {len(strategic_clusters)} strategic, {self.stats['non_strategic_count']} filtered out")
    ...
    async def _get_pending_clusters(
        ...
        """Get clusters with strategic_status = 'pending' and minimum thresholds for conservative processing"""

        with get_db_session() as session:
            # Conservative pre-filtering: Only clusters with ≥4 articles and ≥3 sources
            query = text(
                """
                SELECT 
                    ac.cluster_id, 
                ...
                JOIN article_cluster_members acm ON ac.cluster_id = acm.cluster_id
                JOIN articles a ON acm.article_id = a.id
                WHERE ac.strategic_status = 'pending'
                  AND ac.size >= 4  -- Conservative minimum: at least 4 articles
                GROUP BY ac.cluster_id, ac.label, ac.top_topics, ac.size
                HAVING COUNT(DISTINCT a.source_name) >= 3  -- Conservative minimum: at least 3 sources
                ORDER BY ac.size DESC, COUNT(DISTINCT a.source_name) DESC
            """
            )
