...
def load_inputs(conn, hours_back=72, lang="EN", use_triads=0, use_hub_assist=0, use_clean_events=0):
    """Load all required inputs for pipeline processing."""
    logger.info("Loading pipeline inputs...")
    
    inputs = {}
    
    # Load candidates (articles with core keywords)
    inputs['candidates'] = get_articles_with_core_keywords(conn, hours_back, lang)
    logger.info(f"Loaded {len(inputs['candidates'])} candidate articles")
    
    # Load hub tokens
    inputs['hubs'] = load_hub_tokens(conn)
    logger.info(f"Loaded {len(inputs['hubs'])} hub tokens")
    
    # Load event signals
    inputs['event_signals'] = load_event_signals(conn)
    logger.info(f"Loaded {len(inputs['event_signals'])} event signals")
    
    # Load triads if enabled
    inputs['triads'] = {}
    if use_triads or use_hub_assist:
        inputs['triads'] = load_event_anchored_triads(conn, use_clean_events)
        logger.info(f"Loaded {len(inputs['triads'])} event-anchored triads")
    
    return inputs


def seed_strict(candidates, hub_tokens, event_signals, profile="strict"):
    """Generate strict base seeds using existing logic."""
    logger.info(f"Generating strict seeds (profile: {profile})")
    
    # Use existing topic-based seed generation logic
    topic_seeds = defaultdict(list)
    
    for article in candidates:
        article_id = article['id']
        core_tokens = article['keywords']
        
        # Filter out hub tokens for seed generation
        filtered_tokens = [t for t in core_tokens if t not in hub_tokens]
        if len(filtered_tokens) < 2:
            continue
            
        # Generate topic combinations (existing logic)
        for combo_size in [3, 2]:
            for combo in combinations(sorted(filtered_tokens[:5]), combo_size):
                topic_key = generate_topic_key(combo)
                topic_seeds[topic_key].append(article_id)
    
    # Convert to seed format
    seeds = {}
    seed_count = 0
    for topic_key, article_ids in topic_seeds.items():
        if len(article_ids) >= 2:  # Minimum seed size
            seed_count += 1
            seeds[f"seed_{seed_count:04d}"] = {
                'members': article_ids,
                'topic_key': topic_key,
                'seed_type': 'strict'
            }
    
    logger.info(f"Generated {len(seeds)} strict seeds from {len(topic_seeds)} topics")
    return seeds


def seed_hub_assist(candidates, hub_tokens, triads, event_signals, hub_pair_cos=0.90, use_hub_assist=1):
    """Generate hub-assisted seeds if enabled."""
    if not use_hub_assist:
        logger.info("Hub assist disabled, skipping hub-assisted seed generation")
        return {}
        
    logger.info(f"Generating hub-assisted seeds (cos_threshold: {hub_pair_cos})")
    
    # Load triad pairs for hub assistance
    try:
        from etl_pipeline.core.config import get_db_connection
        ...
        logger.warning(f"Could not load triad pairs for hub assist: {e}")
        triad_pairs = []
    
    hub_seeds = {}
    seed_count = 0
    
    # Simple hub-pair seed generation based on existing logic
    for article in candidates:
        article_tokens = set(article['keywords'])
        
        # Check if article matches triad patterns
        if triad_seed_ok(article_tokens, hub_tokens, triad_pairs):
            seed_count += 1
            hub_seeds[f"hub_seed_{seed_count:04d}"] = {
                'members': [article['id']],
                'hub_tokens': article_tokens & hub_tokens,
                'seed_type': 'hub_assist'
            }
    
    logger.info(f"Generated {len(hub_seeds)} hub-assisted seeds")
    return hub_seeds


def densify_clusters(conn, seeds, hours_back=72, lang="EN", cos_threshold=0.88, profile="strict", use_hub_assist=0, hub_pair_cos=0.90):
    """Densify seeds into full clusters using existing logic."""
    logger.info(f"Densifying {len(seeds)} seeds (cos_threshold: {cos_threshold})")
    
    if not seeds:
        logger.info("No seeds to densify")
        return {}
    
    # Use existing densify logic but with in-memory seeds
    densified = {}
    
    for seed_id, seed_data in seeds.items():
        members = seed_data['members']
        
        # Find similar articles for each seed member
        expanded_members = set(members)
        
        for member_id in members:
            similar_articles = find_similar_articles(
                conn, member_id, hours_back, lang, cos_threshold, limit=50
            )
            
            for similar_id, similarity in similar_articles:
                if similarity >= cos_threshold:
                    expanded_members.add(similar_id)
        
        if len(expanded_members) >= 2:  # Minimum cluster size
            densified[seed_id] = {
                'members': list(expanded_members),
                'seed_type': seed_data.get('seed_type', 'unknown'),
                'original_size': len(members),
                'densified_size': len(expanded_members)
            }
    
    logger.info(f"Densified to {len(densified)} clusters with average size {sum(len(c['members']) for c in densified.values()) / len(densified) if densified else 0:.1f}")
    return densified


def orphan_attach(conn, clusters, hub_tokens, hours_back=72, cos_threshold=0.89, min_shared=1):
    """Attach orphaned articles to existing clusters."""
    if not clusters:
        logger.info("No clusters available for orphan attachment")
        return clusters
        
    logger.info(f"Starting orphan attachment (cos_threshold: {cos_threshold})")
    
    # Get clustered articles
    clustered_articles = set()
    for cluster_data in clusters.values():
        clustered_articles.update(cluster_data['members'])
    
    # Load orphan articles
    orphans = []
    cur = conn.cursor()
    try:
        query = """
        ...
        for article_id, title, source, keywords in cur.fetchall():
            if article_id not in clustered_articles:
                orphans.append({
                    "id": article_id,
                    "title": title or "",
                    "source": source,
    ...
    # Simple attachment based on keyword overlap
    attachments = 0
    for orphan in orphans:
        ...
        for cluster_id, cluster_data in clusters.items():
            # Get cluster keywords (simplified)
            cluster_keywords = set()
            for member_id in cluster_data['members'][:5]:  # Sample for keywords
                cur = conn.cursor()
                try:
                    cur.execute("SELECT token FROM article_core_keywords WHERE article_id = %s", (member_id,))
                    cluster_keywords.update(row[0] for row in cur.fetchall())
                finally:
                    cur.close()
            
            # Calculate keyword overlap (non-hub)
            orphan_nonhub = orphan["keywords"] - hub_tokens
            cluster_nonhub = cluster_keywords - hub_tokens
            shared_nonhub = len(orphan_nonhub & cluster_nonhub)
            
            if shared_nonhub >= min_shared and shared_nonhub > best_score:
                best_score = shared_nonhub
                best_cluster = cluster_id
        
        if best_cluster:
            clusters[best_cluster]['members'].append(orphan['id'])
            attachments += 1
    
    logger.info(f"Attached {attachments} orphan articles to existing clusters")
    return clusters


def classify_macro(clusters, macro_enable=1):
    """Apply macro classification if enabled."""
    if not macro_enable:
        logger.info("Macro classification disabled")
        return clusters
        
    logger.info("Applying macro classification")
    
    # Simple size-based macro classification
    macro_count = 0
    for cluster_id, cluster_data in clusters.items():
        cluster_size = len(cluster_data['members'])
        
        # Mark as macro if cluster is large
        if cluster_size >= 10:
            cluster_data['is_macro'] = True
            macro_count += 1
        else:
            cluster_data['is_macro'] = False
    
    logger.info(f"Classified {macro_count} clusters as macro")
    return clusters


def persist_clusters(conn, clusters, macro_enable=1):
    """Write clusters to database."""
    if not clusters:
        logger.info("No clusters to persist")
        return
        
    logger.info(f"Persisting {len(clusters)} clusters to database")
    
    # Convert pipeline format to stage_persist expected format
    seeds_for_persist = {}
    for cluster_id, cluster_data in clusters.items():
        seeds_for_persist[cluster_id] = {
            'members': cluster_data['members'],
            'seed_type': cluster_data.get('seed_type', 'pipeline'),
            'is_macro': cluster_data.get('is_macro', False)
        }
    
    # Use existing persist logic
    stage_persist(conn, seeds_for_persist, macro_enable)
    
    logger.info("Cluster persistence completed")


def run_pipeline_mode(args):
    """Execute complete pipeline in single process."""
    logger.info("Starting CLUST-1 pipeline mode")
    ...
    try:
        ...
        inputs = load_inputs(
            ...
            args.use_triads, 
            args.use_hub_assist, 
            args.use_clean_events
        )
        
        # Generate seeds
        strict_seeds = seed_strict(
            inputs['candidates'], 
            inputs['hubs'], 
        ...
        # Combine seeds
        all_seeds = {**strict_seeds, **hub_seeds}
        logger.info(f"Total seeds: {len(all_seeds)} (strict: {len(strict_seeds)}, hub: {len(hub_seeds)})")
        
        # Densify clusters
        clusters = densify_clusters(
            conn, 
            all_seeds, 
            args.window, 
            args.lang, 
        ...
        # Attach orphans
        clusters = orphan_attach(
            conn, 
            clusters, 
            inputs['hubs'], 
            args.window, 
            0.89,  # orphan cos threshold
            1      # min shared keywords
        )
        
        # Apply macro classification
        clusters = classify_macro(clusters, args.macro_enable)
        
        # Persist to database
        persist_clusters(conn, clusters, args.macro_enable)
        
        # Final KPIs
        total_articles = sum(len(c['members']) for c in clusters.values())
        avg_size = total_articles / len(clusters) if clusters else 0
        macro_clusters = sum(1 for c in clusters.values() if c.get('is_macro', False))
        
        logger.info("PIPELINE COMPLETED SUCCESSFULLY")
        logger.info(f"Candidates: {len(inputs['candidates'])}")
        logger.info(f"Seeds: {len(all_seeds)} (strict: {len(strict_seeds)}, hub: {len(hub_seeds)})")
        logger.info(f"Final clusters: {len(clusters)}")
        logger.info(f"Macro clusters: {macro_clusters}")
        logger.info(f"Total articles clustered: {total_articles}")
...
def main():
    """Main clustering function with mode selection."""
    ...
    parser.add_argument(
        ...
        default="pipeline",
        help="Execution mode: pipeline (single-process) or stage (individual stages for debugging)",
    )
    parser.add_argument(
        "--stage",
        choices=["seed", "densify", "consolidate", "refine", "persist"],
        help="Clustering stage to run (only used in stage mode)",
    )
    parser.add_argument(
    ...
    if args.mode == "stage" and not args.stage:
        parser.error("--stage is required when using --mode stage")
    if args.mode == "pipeline" and args.stage:
        logger.warning("--stage argument ignored in pipeline mode")

    logger.info(
        f"Starting CLUST-1 {args.mode} mode (language: {args.lang}, MVP English-only)"
    )

    try:
        # Execute based on mode
        if args.mode == "pipeline":
            success = run_pipeline_mode(args)
            if success:
                logger.info("CLUST-1 pipeline completed successfully")
            else:
                logger.error("CLUST-1 pipeline failed")
                sys.exit(1)
        
        elif args.mode == "stage":
            # Keep existing stage-by-stage logic for debugging
            conn = get_db_connection()
            
            try:
                logger.info(f"Running individual stage: {args.stage}")
                ...
                conn.rollback()
                logger.error(f"CLUST-1 stage {args.stage} failed: {e}")
                raise
            finally:
                conn.close()

    except Exception as e:
        logger.error(f"Failed to run CLUST-1: {e}")
        sys.exit(1)
