...
class AdvancedCanonicalizer:
    ...
    def _load_config(self) -> Dict:
        """Load YAML configuration file (optional - uses hardcoded defaults if missing)."""
        try:
            with open(self.config_path, "r", encoding="utf-8") as f:
                return yaml.safe_load(f)
        except Exception:
            # Config file is optional - canonicalizer works with hardcoded defaults
            logger.debug(f"No config file found at {self.config_path}, using hardcoded defaults")
            return {}
