...
class KeywordNormalizer:
    ...
    def _load_config(self) -> Dict:
        """Load YAML configuration file (optional - works without config)."""
        try:
            with open(self.config_path, "r", encoding="utf-8") as f:
                return yaml.safe_load(f)
        except Exception:
            # Config file is optional - normalizer works with empty config
            logger.debug(f"No config file found at {self.config_path}, using empty config")
            return {}
