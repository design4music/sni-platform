...
class RAIOverlayProcessor:
    ...
    def check_analysis_idempotency(self, narrative: Dict) -> bool:
        ...
        if current_hash != stored_hash:
            logger.debug(
                f"Input changed for narrative {narrative['id']}, re-analysis needed"
            )
    ...
    def process_rai_analysis(self, narrative: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            rai_analysis = {
                ...
                "publication_recommendation": self.generate_publication_recommendation(
                    analysis_result
                ),
            ...
            logger.warning(
                f"RAI service unavailable for narrative {narrative['id']}: {e}"
            )
    ...
    def generate_publication_recommendation(self, analysis: Dict) -> str:
        ...
        if (
            overall >= self.min_overall_rai_score
            and adequacy >= self.min_adequacy_score
        ):
            return "approve"
    ...
    def update_narrative_with_rai(
        self, conn, narrative_id: int, rai_analysis: Dict
    ) -> bool:
        ...
        try:
            ...
            new_status.update(
                {
                    ...
                    "gen": {
                        **new_status.get("gen", {}),
                        ...
                        "publication_ready": new_status.get("gen", {}).get(
                            "publication_ready", True
                        ),  # Keep existing ready status
                    },
                    "status": (
                        "ok"
                        if rai_analysis["overall_score"] >= self.min_overall_rai_score
                        else "needs_review"
                    ),
                    "last_rai": now_iso,
                    "rai_version": "1",
            ...
            gen3_entry = {
                ...
                "publication_recommendation": rai_analysis[
    ...
    def process_narrative(self, conn, narrative: Dict) -> bool:
        ...
        try:
            ...
            if not rai_analysis:
                logger.error(
                    f"Failed to generate RAI analysis for narrative {narrative_id}"
                )
                self.error_count += 1
                return False
            ...
            if adequacy < 0.3:
                logger.warning(
                    f"Very low adequacy score ({adequacy}) for narrative {narrative_id}"
                )

            if overall < 0.3:
                logger.warning(
                    f"Very low overall RAI score ({overall}) for narrative {narrative_id}"
                )
            ...
            if self.update_narrative_with_rai(conn, narrative_id, rai_analysis):
                ...
                service_used = (
                    "RAI-Service"
                    if rai_analysis["service_analysis"]
                    else "Local-Fallback"
                )
                logger.info(
                    f"SUCCESS: RAI analysis for narrative {narrative_id} (overall: {overall}, rec: {recommendation}, via: {service_used})"
                )
                return True
            else:
                self.error_count += 1
    ...
    def run_rai_overlay(self) -> Dict:
        ...
        try:
            ...
            if self.rai_client.enabled:
                ...
                logger.info(
                    f"RAI service health: {'OK' if service_healthy else 'UNAVAILABLE'}"
                )
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force regeneration of all RAI analyses"
    )
