...
class FeedExtractorLearner:
    ...
    def analyze_html_structure(self, html: str, url: str) -> Dict:
        ...
        try:
            ...
            candidates = sorted(
                candidates, key=lambda x: x["text_length"], reverse=True
            )[:5]
    ...
    async def get_llm_profile(
        self, structure_summaries: List[Dict], domain: str
    ) -> Optional[Dict]:
        ...
        try:
            ...
            # Add best IDs
            if id_scores:
                best_ids = sorted(id_scores.items(), key=lambda x: x[1], reverse=True)[
                    :2
                ]
            ...
            # Analyze common junk patterns - enhanced intelligence
            junk_phrases = [
                "Subscribe",
                "Privacy Policy", 
                "Cookie Policy",
            ...
            if "presstv" in domain.lower():
                ...
                # Remove Japan Times subscription notice specifically
                junk_phrases.extend([
                    ...
                    "Profile section of your subscriber account page"
                ])
                post_clean_patterns.append({
                    "pattern": r"With your current subscription plan.*?subscriber account page\.", 
                    "flags": "s"
                })
                ...
                # Remove SCMP advertisement and audio controls
                remove_selectors.extend([".audio-player", ".voice-control", ".ad-container"])
                junk_phrases.extend([
                    "Advertisement",
                    "Select Voice",
                    "Choose your listening speed",
                    "Get through articles 2x faster",
                    "250 WPM Slow Average Fast"
                ])
                post_clean_patterns.extend([
                    {"pattern": r"Advertisement\s*Advertisement.*?1\.25 x", "flags": "s"},
                    {"pattern": r"Select Voice.*?1\.25 x", "flags": "s"},
                    {"pattern": r"^\s*Advertisement\s*$", "flags": "m"}
                ])
                
            elif "timesofindia" in domain.lower():
                main_selectors.insert(0, ".article-content")
                main_selectors.insert(0, ".story-content")
                # Remove Times of India language selector and navigation junk
                remove_selectors.extend([".lang-selector", ".language-menu", ".edition-selector"])
                junk_phrases.extend([
                    "Edition IN IN US English",
                    "हिन्दी मराठी ಕನ್ನಡ தமிழ் বাংলা മലയാളം తెలుగు ગુજરાતી",
                    "Weather Sign In News Sports News",
                    "Times of India"
                ])
                post_clean_patterns.extend([
                    {"pattern": r"Edition IN IN US English.*?Sports News", "flags": "s"},
                    {"pattern": r"हिन्दी.*?ગુજરાતી", "flags": "s"},
                    {"pattern": r"Weather Sign In.*?News", "flags": ""}
                ])
            ...
            # Build profile with enhanced intelligence
            profile = self.get_profile_schema()
            profile.update(
                {
                    "main_selector": ", ".join(main_selectors[:5]),  # Top 5 selectors
                    "remove_selectors": remove_selectors,  # Use enhanced removal selectors
                    "junk_phrases": junk_phrases,  # Use enhanced junk phrases
                    "pre_clean_regex": [
                        {"pattern": r"^Share\s+.*", "flags": "m"},
                        {"pattern": r"^Follow us.*", "flags": "m"},
                    ],
                    "post_clean_regex": post_clean_patterns,  # Use enhanced post-cleaning
                    "min_length": 200,  # Slightly higher for news content
                    "density_threshold": 0.08,  # Lower threshold for news sites
                    "notes": f"Enhanced learning from {len(structure_summaries)} pages for {domain} with intelligent junk filtering",
                    "scope": "domain" if len(structure_summaries) >= 6 else "feed",
                }
            )

            logger.info(
                f"Generated profile with main selector: {profile['main_selector']}"
            )
            return profile
    ...
    def validate_profile(
        self, profile: Dict, html_samples: List[Tuple[str, str]]
    ) -> Tuple[bool, Dict]:
    ...
    def save_profile(
        self, profile: Dict, feed_id: Optional[str] = None, domain: Optional[str] = None
    ):
        """Save profile to database"""
        with get_db_session() as db:
            if feed_id:
                query = text(
                    "UPDATE news_feeds SET extraction_profile = :profile WHERE id = :feed_id"
                )
                ...
                db.execute(
                    query,
                    {"profile": json.dumps(profile), "domain_pattern": f"%{domain}%"},
                )
    ...
    async def learn_profile(
        ...
        logger.info(
            f"Learning extraction profile for {'feed ' + feed_id if feed_id else 'domain ' + domain}"
        )

        # Get recent articles
        articles = self.get_recent_articles(
            feed_id, domain, days=14, limit=sample_size * 3
        )
        ...
        for result in validation_results["results"]:
            ...
            print(
                f"  {status:4} | {length:4d} chars | {density:.3f} density | {result['url'][:60]}"
            )
            if error:
                print(f"       | Error: {error}")
        ...
        if passes:
            ...
            logger.info(
                f"Profile saved successfully for {'feed ' + feed_id if feed_id else 'domain ' + domain}"
            )
            return True
        else:
            logger.error(
                f"Profile validation failed (success rate: {validation_results['success_rate']:.1%})"
            )
...
async def main():
    parser = argparse.ArgumentParser(
        description="Learn extraction profiles for news feeds"
    )
    ...
    parser.add_argument(
        "--sample", type=int, default=8, help="Number of pages to sample (5-10)"
    )
    parser.add_argument("--llm", default="deepseek", help="LLM to use for learning")
    parser.add_argument(
        "--dry-run", action="store_true", help="Show proposed JSON but don't save"
    )
