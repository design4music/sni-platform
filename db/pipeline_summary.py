...
with conn.cursor() as cur:
    ...
    cur.execute("SELECT COUNT(*) FROM titles_v3 WHERE processing_status = 'blocked_stopword'")
    blocked_stopword = cur.fetchone()[0]

    cur.execute("SELECT COUNT(*) FROM titles_v3 WHERE processing_status = 'blocked_llm'")
    blocked_llm = cur.fetchone()[0]

    cur.execute("SELECT COUNT(*) FROM titles_v3 WHERE processing_status = 'out_of_scope'")
    oos = cur.fetchone()[0]

print("Phase 1-2 (Ingestion + Centroid Matching):")
print(f"  Total titles ingested: {total_titles}")
print(f"  Matched to centroids: {assigned + blocked_llm} ({(assigned + blocked_llm) / total_titles * 100:.1f}%)")
...
print(f"  Titles with strategic value: {titles_with_tracks} ({titles_with_tracks / (assigned + blocked_llm) * 100:.1f}%)")
print(f"  Rejected by LLM intel gating: {blocked_llm}")
print(f"  Total track assignments: {total_assignments}")
print(f"  Avg assignments per strategic title: {total_assignments / titles_with_tracks:.2f}")
print()
...
print(f"  Titles assigned to multiple centroids: {multi_titles} ({multi_titles / titles_with_tracks * 100:.1f}%)")
