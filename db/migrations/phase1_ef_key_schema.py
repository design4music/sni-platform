...
def verify_migration():
    ...
    with get_db_session() as session:
        ...
        # Check constraints
        constraints_result = session.execute(
            text(
                """
            SELECT conname 
            FROM pg_constraint 
        ...
        for row in sample_result:
            logger.info(
                f"  ID: {row.id}, Theater: {row.primary_theater}, Type: {row.event_type}, Status: {row.status}, EF_Key: {row.ef_key}"
            )
