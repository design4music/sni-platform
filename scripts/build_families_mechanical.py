"""
Mechanical family assembly using title_labels signal aggregation.

Pipeline:
  1. Aggregate title_labels per cluster -> signal profile
  2. Detect ubiquitous signals (>15% of CTM titles)
  3. Assign spine per cluster (place > person > org > subject > bucket_key)
  4. Group clusters by spine -> proto-families
  5. Within-family Dice merge
  6. Priority tagging (A/B/C/D)
  7. Report (dry-run) or write to DB

Usage:
  python scripts/build_families_mechanical.py AMERICAS-USA geo_security
  python scripts/build_families_mechanical.py AMERICAS-USA geo_security --write
"""

import re
import sys
from collections import Counter, defaultdict

import psycopg2

sys.path.insert(0, ".")
sys.stdout.reconfigure(errors="replace")

MONTH = "2026-03-01"
CENTROID = sys.argv[1] if len(sys.argv) > 1 else "AMERICAS-USA"
TRACK = sys.argv[2] if len(sys.argv) > 2 else "geo_security"
WRITE_DB = "--write" in sys.argv
DB_URL = next((a.split("=", 1)[1] for a in sys.argv if a.startswith("--db=")), None)
DB_URL = DB_URL or "postgresql://postgres:postgres@localhost:5432/sni_v2"

# Spine priority by track (which entity type is most discriminating)
TRACK_ENTITY_PRIORITY = {
    "geo_security": ["place", "person", "org"],
    "geo_politics": ["person", "org", "place"],
    "geo_economy": ["org", "person", "place"],
    "geo_society": ["person", "org", "place"],
}

# Vague place names that are too broad to be spines
VAGUE_PLACES = frozenset(
    [
        "middle east",
        "gulf",
        "asia",
        "europe",
        "africa",
        "americas",
        "the west",
        "the east",
    ]
)

# Minimum fraction of a cluster's titles that must mention a signal for it to be "defining"
DEFINING_THRESHOLD = 0.35

# Ubiquity: if a signal appears in >15% of ALL CTM titles, it's ubiquitous
UBIQUITY_THRESHOLD = 0.15

STOP_WORDS = frozenset(
    "the a an in of on for to and is are was were with from at by as its it be "
    "has had have that this or but not no new over after into about up out more "
    "says said will could would may amid us set than been also".split()
)


def tokenize(text):
    words = set(re.findall(r"[a-z][a-z0-9]+", (text or "").lower()))
    return words - STOP_WORDS


def dice(a, b):
    if not a or not b:
        return 0.0
    return 2 * len(a & b) / (len(a) + len(b))


# -------------------------------------------------------------------
# Step 1: Load clusters and aggregate title_labels
# -------------------------------------------------------------------


def load_clusters(cur, ctm_id):
    """Load all non-catchall visible clusters with their title_labels aggregated."""

    # Get cluster basics
    cur.execute(
        """
        SELECT e.id, e.source_batch_count, e.title, e.event_type, e.bucket_key, e.tags
        FROM events_v3 e
        WHERE e.ctm_id = %s AND NOT e.is_catchall AND e.merged_into IS NULL
        ORDER BY e.source_batch_count DESC
    """,
        (ctm_id,),
    )

    clusters = {}
    for r in cur.fetchall():
        clusters[r[0]] = {
            "id": r[0],
            "src": r[1],
            "title": r[2] or "",
            "event_type": r[3],
            "bucket_key": r[4],
            "tags": set(r[5] or []),
            "words": tokenize(r[2]),
            # Will be filled by aggregation:
            "n_titles": 0,
            "max_importance": 0.0,
            "avg_importance": 0.0,
            "places": Counter(),
            "persons": Counter(),
            "orgs": Counter(),
            "subjects": Counter(),
            "actors": Counter(),
            "targets": Counter(),
            "action_classes": Counter(),
            "named_events": Counter(),
        }

    if not clusters:
        return clusters

    # Aggregate title_labels per cluster
    cluster_ids = list(clusters.keys())
    cur.execute(
        """
        SELECT et.event_id,
            tl.importance_score,
            tl.actor, tl.target, tl.subject, tl.action_class,
            tl.places, tl.persons, tl.orgs, tl.named_events
        FROM event_v3_titles et
        JOIN title_labels tl ON tl.title_id = et.title_id
        WHERE et.event_id = ANY(%s::uuid[])
    """,
        ([str(cid) for cid in cluster_ids],),
    )

    for r in cur.fetchall():
        eid = r[0]
        c = clusters.get(eid)
        if not c:
            continue

        c["n_titles"] += 1
        imp = r[1] or 0.0
        if imp > c["max_importance"]:
            c["max_importance"] = imp

        if r[2]:  # actor
            c["actors"][r[2]] += 1
        if r[3]:  # target
            c["targets"][r[3]] += 1
        if r[4]:  # subject
            c["subjects"][r[4]] += 1
        if r[5]:  # action_class
            c["action_classes"][r[5]] += 1
        for p in r[6] or []:  # places
            c["places"][p] += 1
        for p in r[7] or []:  # persons
            c["persons"][p] += 1
        for o in r[8] or []:  # orgs
            c["orgs"][o] += 1
        for ne in r[9] or []:  # named_events
            c["named_events"][ne] += 1

    # Compute avg importance
    for c in clusters.values():
        if c["n_titles"] > 0:
            # We already summed max; for avg, re-query would be expensive.
            # Use a simple heuristic: avg ~ max * 0.6 for large clusters
            c["avg_importance"] = c["max_importance"] * 0.6

    return clusters


# -------------------------------------------------------------------
# Step 2: Detect ubiquitous signals
# -------------------------------------------------------------------


def detect_ubiquitous(clusters):
    """Find signals that appear in >15% of all titles across the CTM."""
    total_titles = sum(c["n_titles"] for c in clusters.values())
    if total_titles == 0:
        return set()

    threshold = total_titles * UBIQUITY_THRESHOLD

    # Count across all titles (not clusters)
    person_totals = Counter()
    place_totals = Counter()
    org_totals = Counter()

    for c in clusters.values():
        for p, cnt in c["persons"].items():
            person_totals[p] += cnt
        for p, cnt in c["places"].items():
            place_totals[p] += cnt
        for o, cnt in c["orgs"].items():
            org_totals[o] += cnt

    ubiquitous = set()
    for p, cnt in person_totals.items():
        if cnt >= threshold:
            ubiquitous.add(("person", p))
    for p, cnt in place_totals.items():
        if cnt >= threshold:
            ubiquitous.add(("place", p))
    for o, cnt in org_totals.items():
        if cnt >= threshold:
            ubiquitous.add(("org", o))

    return ubiquitous


# -------------------------------------------------------------------
# Step 2b: Normalize place aliases (substring containment)
# -------------------------------------------------------------------


def normalize_places(clusters):
    """Merge place aliases where one name is a clear variant of another.

    Rules: only merge if the shorter form is a PREFIX or SUFFIX of the longer,
    AND both forms refer to the same place (not Romania/Oman, Kharkiv/Kharg).
    """
    # Collect all place names across all clusters
    all_places = Counter()
    for c in clusters.values():
        for p, cnt in c["places"].items():
            all_places[p] += cnt

    # Case normalization first (NEW YORK -> New York)
    case_map = {}
    by_lower = defaultdict(list)
    for p in all_places:
        by_lower[p.lower()].append(p)
    for lower, variants in by_lower.items():
        if len(variants) > 1:
            # Keep the variant with most occurrences
            best = max(variants, key=lambda v: all_places[v])
            for v in variants:
                if v != best:
                    case_map[v] = best

    # Structural aliases: "X Island" -> "X", "X City" -> "X", "X Port" -> "X"
    # Only if the base form also exists
    suffixes = [
        " Island",
        " City",
        " Port",
        " Airport",
        " Strait",
        " Straits",
        " Air Base",
        " D.C.",
        " DC",
    ]
    struct_map = {}
    place_lower_set = {p.lower() for p in all_places}

    for p in all_places:
        for suffix in suffixes:
            if p.endswith(suffix):
                base = p[: -len(suffix)].strip()
                if base.lower() in place_lower_set:
                    struct_map[p] = base
                    break

    alias_map = {**case_map, **struct_map}
    if alias_map:
        print("  Place aliases: %s" % {k: v for k, v in sorted(alias_map.items())})

    # Apply aliases to all clusters
    for c in clusters.values():
        merged = Counter()
        for p, cnt in c["places"].items():
            canonical = alias_map.get(p, p)
            merged[canonical] += cnt
        c["places"] = merged


# -------------------------------------------------------------------
# Step 3: Assign spine per cluster
# -------------------------------------------------------------------


def assign_spines(clusters, ubiquitous, track, cur=None):
    """Assign a spine to each cluster based on its signal profile."""

    entity_priority = TRACK_ENTITY_PRIORITY.get(track, ["place", "person", "org"])

    # Build set of country/region names from centroids -- these are too broad
    # to be place-spines when the cluster already has a bucket_key
    country_names = set()
    if cur:
        cur.execute("SELECT id, label FROM centroids_v3 WHERE is_active")
        for row in cur.fetchall():
            country_names.add(row[1].lower())  # "United States", "Iran", etc.
            # Also add short forms derived from centroid key
            parts = row[0].split("-")
            if len(parts) >= 2:
                country_names.add(parts[-1].lower())  # "usa", "iran", "france"
    # Common country names that appear as extracted places
    country_names.update(
        [
            "spain",
            "france",
            "germany",
            "italy",
            "greece",
            "turkey",
            "japan",
            "india",
            "china",
            "russia",
            "brazil",
            "mexico",
            "canada",
            "australia",
            "romania",
            "poland",
            "norway",
            "sweden",
            "denmark",
            "finland",
            "pakistan",
            "afghanistan",
            "syria",
            "egypt",
            "libya",
            "morocco",
            "singapore",
            "vietnam",
            "philippines",
            "indonesia",
            "thailand",
            "colombia",
            "peru",
            "chile",
            "argentina",
            "nigeria",
            "kenya",
            "united states",
            "united kingdom",
            "south korea",
            "north korea",
            "saudi arabia",
            "united arab emirates",
        ]
    )

    for c in clusters.values():
        n = c["n_titles"]
        if n == 0:
            n = max(c["src"], 1)

        import math

        min_count = max(2, math.ceil(n * DEFINING_THRESHOLD))

        # 1. Named events (rare but definitive)
        best_ne = _best_signal(c["named_events"], min_count, set())
        if best_ne:
            c["spine_type"] = "named_event"
            c["spine_value"] = best_ne
            continue

        # 2-4. Entity types in track-specific priority order
        # For places: exclude country-level names that DON'T match the cluster's
        # own bucket_key. A cluster about US-Iran with "Spain" mentioned is NOT
        # a Spain story. But a cluster about US-Iran with "Kuwait" as the
        # dominant place IS a Kuwait-specific story (friendly fire incident).
        #
        # Rule: exclude a country name ONLY if it's mentioned in <40% of titles
        # (incidental mention) AND doesn't match the bucket_key region.
        place_excluded = {p for t, p in ubiquitous if t == "place"} | VAGUE_PLACES
        if c["bucket_key"]:
            bucket_lower = c["bucket_key"].lower().replace("-", " ")
            for place_name, place_count in list(c["places"].items()):
                pn = place_name.lower()
                # Skip if the place IS the bucket target (always relevant)
                if pn in bucket_lower or bucket_lower.endswith(pn):
                    continue
                # If it's a country name and incidental (<40% of titles), exclude
                if pn in country_names and n > 0 and place_count < n * 0.4:
                    place_excluded.add(pn)

        entity_sources = {
            "place": (c["places"], place_excluded),
            "person": (c["persons"], {p for t, p in ubiquitous if t == "person"}),
            "org": (c["orgs"], {p for t, p in ubiquitous if t == "org"}),
        }

        spine_found = False
        for etype in entity_priority:
            signals, excluded = entity_sources[etype]
            best = _best_signal(signals, min_count, excluded)
            if best:
                c["spine_type"] = etype
                c["spine_value"] = best
                spine_found = True
                break

        if spine_found:
            continue

        # 5. Subject as compound spine (with bucket_key or event_type prefix)
        # Filter out overly generic subjects
        generic_subjects = {
            "BILATERAL_RELATIONS",
            "DEFENSE_POLICY",
            "MEDIA_PRESS",
            "EXECUTIVE_ACTION",
        }
        best_subj = _best_signal(c["subjects"], min_count, generic_subjects)

        prefix = c["bucket_key"] or c["event_type"] or "unknown"

        if best_subj:
            c["spine_type"] = "compound"
            c["spine_value"] = "%s:%s" % (prefix, best_subj)
            continue

        # 6. Bucket key alone (for small bilateral sets)
        if c["bucket_key"]:
            c["spine_type"] = "bucket"
            c["spine_value"] = c["bucket_key"]
            continue

        # 7. Event type alone (last resort)
        c["spine_type"] = "type"
        c["spine_value"] = c["event_type"] or "unknown"


def _best_signal(counter, min_count, excluded):
    """Pick the most frequent non-excluded signal above min_count."""
    for signal, count in counter.most_common():
        if count < min_count:
            return None
        if signal.lower() in excluded or signal in excluded:
            continue
        return signal
    return None


# -------------------------------------------------------------------
# Step 4: Group into families
# -------------------------------------------------------------------


def build_families(clusters):
    """Group clusters by spine into proto-families."""

    spine_groups = defaultdict(list)
    for c in clusters.values():
        key = (c.get("spine_type", "none"), c.get("spine_value", "none"))
        spine_groups[key].append(c)

    families = []
    for (stype, svalue), members in spine_groups.items():
        members.sort(key=lambda c: -c["src"])
        total_src = sum(c["src"] for c in members)
        families.append(
            {
                "spine_type": stype,
                "spine_value": svalue,
                "clusters": members,
                "n_clusters": len(members),
                "total_src": total_src,
            }
        )

    # Sort families by total source count
    families.sort(key=lambda f: -f["total_src"])

    # Within-family Dice merge (lower threshold since same spine)
    for f in families:
        if f["n_clusters"] < 2:
            continue
        f["clusters"] = _merge_within_family(f["clusters"])
        f["n_clusters"] = len(f["clusters"])

    # Split oversized families (>20 clusters or >15 clusters with >500 src)
    final_families = []
    for f in families:
        if f["n_clusters"] > 20 or (f["n_clusters"] > 15 and f["total_src"] > 500):
            if f["spine_type"] in ("compound", "bucket", "type"):
                sub = _split_by_action_class(f)
            else:
                # Entity-spine families: split by secondary subject
                sub = _split_by_subject(f)
            final_families.extend(sub)
        else:
            final_families.append(f)

    # Dissolve tiny families (< 10 total src) into standalones
    result = []
    for f in final_families:
        if f["n_clusters"] >= 2 and f["total_src"] < 10:
            # Too small to be a meaningful family -- break into standalones
            for c in f["clusters"]:
                result.append(
                    {
                        "spine_type": f["spine_type"],
                        "spine_value": f["spine_value"],
                        "clusters": [c],
                        "n_clusters": 1,
                        "total_src": c["src"],
                    }
                )
        else:
            result.append(f)

    # Absorb standalone topics into families within the same bucket_key
    # A standalone with bucket_key=MIDEAST-IRAN should join the best-matching
    # MIDEAST-IRAN family if title words overlap
    result = _absorb_standalones_into_families(result)

    result.sort(key=lambda f: -f["total_src"])
    return result


def _absorb_standalones_into_families(families):
    """Absorb standalone topics into multi-cluster families in the same bucket.

    A standalone (1-cluster family) with bucket_key X gets absorbed into the
    best-matching multi-cluster family that also has bucket_key X members,
    if title word overlap >= 0.25.
    """
    # Separate standalones from real families
    real = [f for f in families if f["n_clusters"] >= 2]
    solos = [f for f in families if f["n_clusters"] == 1]

    if not real or not solos:
        return families

    # Build word sets per real family (union of all cluster title words)
    fam_words = []
    fam_buckets = []
    for f in real:
        words = set()
        buckets = set()
        for c in f["clusters"]:
            words.update(c["words"])
            if c["bucket_key"]:
                buckets.add(c["bucket_key"])
            if c["event_type"] == "domestic":
                buckets.add("DOMESTIC")
        fam_words.append(words)
        fam_buckets.append(buckets)

    absorbed_count = 0
    remaining_solos = []

    for solo in solos:
        c = solo["clusters"][0]
        solo_bucket = c["bucket_key"] or (
            "DOMESTIC" if c["event_type"] == "domestic" else None
        )
        if not solo_bucket or not c["words"]:
            remaining_solos.append(solo)
            continue

        # Find best matching family in the same bucket
        best_score = 0.0
        best_idx = -1
        for i, f in enumerate(real):
            if solo_bucket not in fam_buckets[i]:
                continue
            if real[i]["n_clusters"] >= 25:
                continue

            # For entity-spine families (place/person/org/named_event):
            # the standalone MUST mention the spine entity in its title or labels
            stype = f["spine_type"]
            sval = f["spine_value"]
            if stype in ("place", "person", "org", "named_event"):
                spine_word = sval.lower().split("/")[0]  # "Hormuz" from "Hormuz/NAVAL"
                # The spine entity must appear in the cluster's title text
                # (not just labels -- labels can have incidental mentions)
                if spine_word not in c["title"].lower():
                    continue

            shared = len(c["words"] & fam_words[i])
            if not c["words"]:
                continue
            score = shared / len(c["words"])
            if score > best_score:
                best_score = score
                best_idx = i

        if best_score >= 0.30 and best_idx >= 0:
            real[best_idx]["clusters"].append(c)
            real[best_idx]["n_clusters"] += 1
            real[best_idx]["total_src"] += c["src"]
            fam_words[best_idx].update(c["words"])
            absorbed_count += 1
        else:
            remaining_solos.append(solo)

    if absorbed_count:
        print("  Absorbed %d standalones into existing families" % absorbed_count)

    return real + remaining_solos


def _merge_within_family(members):
    """Merge near-duplicate clusters within a family (Dice >= 0.45)."""
    if len(members) < 2:
        return members

    kept = []
    absorbed = set()

    for c in members:
        if c["id"] in absorbed:
            continue
        for other in members:
            if other["id"] in absorbed or other["id"] == c["id"]:
                continue
            if dice(c["words"], other["words"]) >= 0.45:
                c["src"] += other["src"]
                c.setdefault("merged_members", []).append(other)
                absorbed.add(other["id"])
        kept.append(c)

    return kept


def _split_by_action_class(family):
    """Split an oversized family into sub-families by dominant action_class."""
    sub_groups = defaultdict(list)
    for c in family["clusters"]:
        top_ac = c["action_classes"].most_common(1)
        ac = top_ac[0][0] if top_ac else "OTHER"
        sub_groups[ac].append(c)

    sub_families = []
    for ac, members in sub_groups.items():
        members.sort(key=lambda c: -c["src"])
        total_src = sum(c["src"] for c in members)
        sub_families.append(
            {
                "spine_type": family["spine_type"],
                "spine_value": "%s/%s" % (family["spine_value"], ac),
                "clusters": members,
                "n_clusters": len(members),
                "total_src": total_src,
            }
        )

    return sub_families


def _split_by_subject(family):
    """Split an oversized entity-spine family by dominant subject."""
    sub_groups = defaultdict(list)
    for c in family["clusters"]:
        top_subj = c["subjects"].most_common(1)
        subj = top_subj[0][0] if top_subj else "OTHER"
        sub_groups[subj].append(c)

    # Only split if it actually reduces size; otherwise keep as-is
    if len(sub_groups) < 2:
        return [family]

    sub_families = []
    for subj, members in sub_groups.items():
        members.sort(key=lambda c: -c["src"])
        total_src = sum(c["src"] for c in members)
        sub_families.append(
            {
                "spine_type": family["spine_type"],
                "spine_value": "%s/%s" % (family["spine_value"], subj),
                "clusters": members,
                "n_clusters": len(members),
                "total_src": total_src,
            }
        )

    return sub_families


# -------------------------------------------------------------------
# Step 5: Priority tagging
# -------------------------------------------------------------------


def assign_priority(clusters):
    """Assign display priority A/B/C/D to each cluster."""
    for c in clusters.values():
        imp = c["max_importance"]
        src = c["src"]
        has_entity = bool(
            c["places"] or any(p for p in c["persons"] if p != "TRUMP") or c["orgs"]
        )

        if imp >= 0.5 or src >= 20:
            c["priority"] = "A"
        elif imp >= 0.3 or src >= 10:
            c["priority"] = "B"
        elif imp >= 0.15 and has_entity:
            c["priority"] = "C"
        else:
            c["priority"] = "D"


# -------------------------------------------------------------------
# Report
# -------------------------------------------------------------------


def print_report(families, clusters):
    """Print detailed report of family assignment."""

    total_clusters = len(clusters)
    total_src = sum(c["src"] for c in clusters.values())

    # Spine type distribution
    spine_dist = Counter()
    for c in clusters.values():
        spine_dist[c.get("spine_type", "none")] += 1

    print("\n=== SPINE ASSIGNMENT ===")
    for stype, cnt in spine_dist.most_common():
        print(
            "  %-15s %4d clusters (%4.1f%%)" % (stype, cnt, 100 * cnt / total_clusters)
        )

    # Priority distribution
    pri_dist = Counter()
    for c in clusters.values():
        pri_dist[c.get("priority", "?")] += 1

    print("\n=== PRIORITY DISTRIBUTION ===")
    for p in ["A", "B", "C", "D"]:
        cnt = pri_dist.get(p, 0)
        print(
            "  Priority %s: %4d clusters (%4.1f%%)"
            % (p, cnt, 100 * cnt / total_clusters)
        )

    # Family report
    real_families = [f for f in families if f["n_clusters"] >= 2]
    standalones = [f for f in families if f["n_clusters"] == 1]

    print(
        "\n=== FAMILIES (%d families, %d standalone) ==="
        % (len(real_families), len(standalones))
    )

    for f in real_families:
        print(
            "\n  [%2d clusters, %5d src] (%s) %s"
            % (f["n_clusters"], f["total_src"], f["spine_type"], f["spine_value"])
        )
        for c in f["clusters"][:5]:
            merged = len(c.get("merged_members", []))
            merged_str = " (+%d merged)" % merged if merged else ""
            print(
                "      %4d src [%s] %s%s"
                % (c["src"], c.get("priority", "?"), c["title"][:60], merged_str)
            )
        if f["n_clusters"] > 5:
            print("      ... +%d more" % (f["n_clusters"] - 5))

    if standalones:
        print("\n=== STANDALONE TOPICS (%d) ===" % len(standalones))
        for f in standalones[:20]:
            c = f["clusters"][0]
            print(
                "  %4d src [%s] (%s:%s) %s"
                % (
                    c["src"],
                    c.get("priority", "?"),
                    f["spine_type"],
                    f["spine_value"][:20],
                    c["title"][:55],
                )
            )
        if len(standalones) > 20:
            print("  ... +%d more" % (len(standalones) - 20))

    # Summary stats
    in_families = sum(f["n_clusters"] for f in real_families)
    in_families_src = sum(f["total_src"] for f in real_families)
    print("\n=== SUMMARY ===")
    print("  Total clusters: %d (%d src)" % (total_clusters, total_src))
    print("  In families: %d clusters (%d src)" % (in_families, in_families_src))
    print("  Standalone: %d clusters" % len(standalones))
    print("  Families: %d" % len(real_families))
    print(
        "  Largest family: %d clusters"
        % max((f["n_clusters"] for f in real_families), default=0)
    )

    # Check success criteria
    print("\n=== CHECKS ===")
    mega = [f for f in real_families if f["n_clusters"] > 15]
    if mega:
        print("  WARNING: %d families >15 clusters:" % len(mega))
        for f in mega:
            print("    %d clusters: %s" % (f["n_clusters"], f["spine_value"]))
    else:
        print("  OK: No mega-families (all <=15 clusters)")

    dominator = [f for f in real_families if f["total_src"] > total_src * 0.25]
    if dominator:
        print("  WARNING: %d families >25%% of sources:" % len(dominator))
        for f in dominator:
            print(
                "    %d src (%.0f%%): %s"
                % (f["total_src"], 100 * f["total_src"] / total_src, f["spine_value"])
            )
    else:
        print("  OK: No family dominates >25%% of sources")


# -------------------------------------------------------------------
# DB Write
# -------------------------------------------------------------------


def write_to_db(conn, cur, ctm_id, families):
    """Write families to event_families table and link clusters via family_id."""
    import uuid

    # Clear old families for this CTM
    cur.execute("UPDATE events_v3 SET family_id = NULL WHERE ctm_id = %s", (ctm_id,))
    cur.execute("DELETE FROM event_families WHERE ctm_id = %s", (ctm_id,))
    print("  Cleared old families")

    real_families = [f for f in families if f["n_clusters"] >= 2]
    written = 0

    for f in real_families:
        fam_id = str(uuid.uuid4())
        fam_src = sum(c["src"] for c in f["clusters"])

        # Use spine_value as title for now (LLM polish would improve this)
        title = f["spine_value"]
        # Compute date range from cluster titles (approximate)
        domain = f["spine_type"]

        cur.execute(
            """INSERT INTO event_families
               (id, ctm_id, title, summary, domain, cluster_count, source_count,
                created_at, updated_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, NOW(), NOW())""",
            (fam_id, ctm_id, title, None, domain, f["n_clusters"], fam_src),
        )

        # Link all clusters in this family
        cluster_ids = [str(c["id"]) for c in f["clusters"]]
        cur.execute(
            "UPDATE events_v3 SET family_id = %s WHERE id = ANY(%s::uuid[])",
            (fam_id, cluster_ids),
        )

        # Also link within-family merged members
        for c in f["clusters"]:
            for merged in c.get("merged_members", []):
                cur.execute(
                    "UPDATE events_v3 SET merged_into = %s, family_id = %s WHERE id = %s",
                    (str(c["id"]), fam_id, str(merged["id"])),
                )

        written += 1

    conn.commit()
    print(
        "  Wrote %d families (%d clusters linked)"
        % (written, sum(f["n_clusters"] for f in real_families))
    )

    # Verify
    cur.execute(
        """
        SELECT count(*) FROM event_families WHERE ctm_id = %s
    """,
        (ctm_id,),
    )
    print("  Verified: %d families in DB" % cur.fetchone()[0])


# -------------------------------------------------------------------
# Main
# -------------------------------------------------------------------


def main():
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor()

    # Get CTM
    cur.execute(
        "SELECT id FROM ctm WHERE centroid_id = %s AND track = %s AND month = %s",
        (CENTROID, TRACK, MONTH),
    )
    row = cur.fetchone()
    if not row:
        print("CTM not found: %s %s %s" % (CENTROID, TRACK, MONTH))
        return
    ctm_id = row[0]

    print("=== %s %s %s ===" % (CENTROID, TRACK, MONTH))

    # Step 1: Load and aggregate
    print("\nStep 1: Loading clusters and aggregating title_labels...")
    clusters = load_clusters(cur, ctm_id)
    print("  Loaded %d visible clusters" % len(clusters))

    with_labels = sum(1 for c in clusters.values() if c["n_titles"] > 0)
    print("  With title_labels: %d" % with_labels)

    # Step 2: Ubiquity detection
    print("\nStep 2: Detecting ubiquitous signals...")
    ubiquitous = detect_ubiquitous(clusters)
    for utype, uval in sorted(ubiquitous):
        print("  Ubiquitous: %s:%s" % (utype, uval))

    # Step 2b: Normalize place aliases
    print("\nStep 2b: Normalizing place aliases...")
    normalize_places(clusters)

    # Step 3: Assign spines
    print("\nStep 3: Assigning spines...")
    assign_spines(clusters, ubiquitous, TRACK, cur)

    # Step 5: Priority (before family assembly, for reporting)
    print("\nStep 5: Assigning priority...")
    assign_priority(clusters)

    # Step 4: Build families
    print("\nStep 4: Building families...")
    families = build_families(clusters)

    # Report
    print_report(families, clusters)

    if WRITE_DB:
        print("\n--- Writing to DB... ---")
        write_to_db(conn, cur, ctm_id, families)

    conn.close()


if __name__ == "__main__":
    main()
