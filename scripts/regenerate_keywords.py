...
def clear_existing_keywords(conn):
    ...
    try:
        ...
        logger.info(f"Cleared {deleted_mappings} article-keyword mappings and {deleted_keywords} keywords")
...
def store_keywords_batch(conn, keyword_results):
    ...
    try:
        ...
        for result in keyword_results:
            ...
            for rank, keyword in enumerate(result.keywords, 1):
                ...
                keyword_id = get_or_create_keyword_with_lifecycle(
                    cur, keyword.text, keyword.keyword_type, keyword.entity_label, 
                    float(keyword.strategic_score), now
                )
        ...
        logger.info(f"Stored keywords with full lifecycle data for {len(keyword_results)} articles")
...
def get_or_create_keyword_with_lifecycle(cur, keyword_text, keyword_type, entity_label, strategic_score, now):
...
def main():
    ...
    try:
        ...
        try:
            ...
            for i, (article_id, title, content, summary, published_at, language) in enumerate(articles):
                try:
                    # English-only filter for MVP
                    article_language = language or "en"
                    if article_language != "en":
                        total_stats['skipped_non_english'] += 1
                        logger.debug(f"Skipped non-English article {article_id} (language: {article_language})")
                        continue
                    ...
                    if total_processed % 100 == 0:
                        logger.info(f"Processed {total_processed}/{len(articles)} articles")
            ...
            try:
                ...
                for keyword, count, score in cur.fetchall():
                    logger.info(f"  {keyword} (articles: {count}, avg_score: {score:.3f})")
