...
def compute_document_frequencies(conn):
    ...
    logger.info(f"Found {len(doc_freq)} unique canonical tokens across {len(article_tokens)} articles")
...
def build_core_keywords(conn, article_tokens, doc_freq, valid_tokens):
    ...
    for article_id, tokens in article_tokens.items():
        ...
        filtered_tokens = [(token, score) for token, score in tokens 
    ...
    pct_with_core = (articles_with_core / total_articles * 100) if total_articles > 0 else 0
    
    logger.info(f"Built article_core_keywords: {len(core_keywords)} mappings")
    logger.info(f"Articles with >=3 core concepts: {articles_with_core}/{total_articles} ({pct_with_core:.1f}%)")
    ...
    for article_id, tokens in articles:
        ...
        for i, token1 in enumerate(tokens):
            for token2 in tokens[i+1:]:
                ...
                if cluster1 == cluster2 or (cluster_counts[cluster1] >= 2 or cluster_counts[cluster2] >= 2):
                    edges += 1
...
def generate_metrics_report(conn, vocab_size, articles_with_core, total_articles, avg_edges):
...
def main():
    ...
    try:
        ...
        try:
            ...
            generate_metrics_report(conn, len(valid_tokens), articles_with_core, 
                                  total_articles, avg_edges)
