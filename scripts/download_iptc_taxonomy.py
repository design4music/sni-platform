    ...
    try:
        ...
        for concept in concepts:
            ...
            if isinstance(pref_label, dict):
                name = pref_label.get('@value') or pref_label.get('en') or str(pref_label)
            ...
            if broader:
                if isinstance(broader, dict):
                    ...
                    parent_id = broader[0] if isinstance(broader[0], str) else broader[0].get('@id')
                elif isinstance(broader, str):
                    parent_id = broader
                
                # Clean parent ID
                if parent_id and parent_id.startswith('http://cv.iptc.org/newscodes/mediatopic/'):
                    parent_id = parent_id.replace('http://cv.iptc.org/newscodes/mediatopic/', '')
